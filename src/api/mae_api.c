...
/*!
***********************************************************
* \brief init_user_functions
* \pre None.
* \post user functions initialized
...
DLLEXPORT int MaeUpdateImage (char *p_InputName, int *p_nNewImage, int nImageWidth, int nImageHeight)
{
	...
	if (p_InputName == NULL)
	{
		ERROR ("Invalid input name (MaeUpdateImage).", "", "");
		return (-1);
	}

	INPUT_DESC *input = get_input_by_name(p_InputName);

	// Cheks the image dimentions
	if ((nImageWidth != input->ww) || (nImageHeight != input->wh))
	{
	...
	for (j = 0; j < nImageHeight; j++)
	{
		for (i = 0; i < nImageWidth; i++)
		{
			...
			input->image[3 * (i + j * input->tfw) + 0] = (GLubyte) RED(nPixel);
			input->image[3 * (i + j * input->tfw) + 1] = (GLubyte) GREEN(nPixel);
			input->image[3 * (i + j * input->tfw) + 2] = (GLubyte) BLUE(nPixel);
		}
	}
	...
	MESSAGE ("MaeUpdateImage", "", "");

	/*
	{
		FILE *pImageFile = fopen (input->name, "w");
		...
		for (j = nImageHeight; j >= 0 ; j--)
        		for (i = 0; i < nImageWidth; i++)
       			      fprintf (pImageFile, "%d %d %d\n",
       			    		  input->image[3 * (i + j * input->tfw) + 0],
							  input->image[3 * (i + j * input->tfw) + 1],
							  input->image[3 * (i + j * input->tfw) + 2]);
		fclose (pImageFile);
	}
	 */
	return (0);
...
XY_PAIR get_xy_pair_from_input_layer (char *input_name)
{
	XY_PAIR		point;
	INPUT_DESC	*input;
...
{
	interpreter (script);
	MESSAGE ("MaeExecuteCommandScript", ":", script);
}
