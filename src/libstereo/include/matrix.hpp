//Header needed by the new versions of GCC (4.3 and followers)
//Eliminate erros such "error: 'memcpy' was not declared on this scope"
//Camilo Alves Carvalho - 02/JUL/2009 (camilo@lcad.inf.ufes.br)
#include <cstring>
...
public:
   ...
   matrixT PseudoInv () _THROW_MATRIX_ERROR;
   matrixT SkewSymetric () _THROW_MATRIX_ERROR;
   matrixT Col (size_t col) _THROW_MATRIX_ERROR;
   matrixT Row (size_t row) _THROW_MATRIX_ERROR;
...
// binary matrix concatenation operator
MAT_TEMPLATE inline matrixT
operator | (const matrixT& m1, const matrixT& m2) _THROW_MATRIX_ERROR
{
   if (m1.RowNo() != m2.RowNo())
      REPORT_ERROR( "matrixT::operator | : Inconsistent matrix sizes in concatenation!");

   matrixT temp(m1.RowNo(),m1.ColNo() + m2.ColNo());
...
matrixT::PseudoInv () _THROW_MATRIX_ERROR
{
   ...
   if (_m->Row < _m->Col)
      REPORT_ERROR( "matrixT::operator!: error while computing the pseudo-inverse, matrix has more columns than rows");
...
matrixT::Col (size_t col) _THROW_MATRIX_ERROR
{
   ...
   if (col < 0 || col >= _m->Col)
      REPORT_ERROR( "matrixT::operator!: matrix column out of bounds.");
...
matrixT::Row (size_t row) _THROW_MATRIX_ERROR
{
   ...
   if (row < 0 || row >= _m->Row)
      REPORT_ERROR( "matrixT::operator!: matrix row out of bounds.");

   matrixT temp(1, _m->Col);

   for (i=0; i < _m->Col; i++)
       temp._m->Val[0][i] = _m->Val[row][i];
...
// calculate skew-symetric matrix of a 3 x 1 matrix
MAT_TEMPLATE inline matrixT
matrixT::SkewSymetric () _THROW_MATRIX_ERROR
{
   if (_m->Row != 3 || _m->Col != 1)
      REPORT_ERROR( "matrixT::SkewSymetric(): Inconsistent matrix size, must be 3x1.");

   matrixT temp(3,3);

   temp._m->Val[0][0] = 0.0; 		temp._m->Val[0][1] = -_m->Val[2][0]; 	temp._m->Val[0][2] = _m->Val[1][0];
   temp._m->Val[1][0] = _m->Val[2][0]; 	temp._m->Val[1][1] = 0.0; 		temp._m->Val[1][2] = -_m->Val[0][0];
   temp._m->Val[2][0] = -_m->Val[1][0]; temp._m->Val[2][1] = _m->Val[0][0]; 	temp._m->Val[2][2] = 0.0;
