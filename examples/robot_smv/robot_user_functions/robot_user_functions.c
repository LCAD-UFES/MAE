...
void set_vergence (void)
{
	...
	for (i = 0; i < (wo * ho); i++)
	{
		...
		nl_mt_gaussian_var.neuron_vector[i].output.fval = (fltSum / (float) nAllSamples) - g_DispData->samples[0][i].val;
	}
...
void set_vergence_coarse2fine (void)
{
	...
	if (nMaxScan > image_right.neuron_layer->dimentions.x)
		nMaxScan = image_right.neuron_layer->dimentions.x;
	nAllSamples = (nMaxScan - nMinScan);
        nVergLeft = nMinScan + (nAllSamples >> 1);
        
        for (PYRAMID_LEVEL = image_left.pyramid_height; PYRAMID_LEVEL >= 0; PYRAMID_LEVEL--)
        {
        	fltMinSum = FLT_MAX;
        	nMinScan = nVergLeft - (nAllSamples >> (image_left.pyramid_height - PYRAMID_LEVEL + 1));
        	nMaxScan = nVergLeft + (nAllSamples >> (image_left.pyramid_height - PYRAMID_LEVEL + 1));
...
XY_PAIR calculateVergencePoint (XY_PAIR g_pInputRightPoint)
{
	...
	switch (VERGENCE_SCAN_TYPE)
	{
                case LINEAR_SCAN:
                        set_vergence ();
                        break;
                case COARSE2FINE_SCAN:
...
void f_keyboard (char *key_value)
{
	...
	switch (key)
	{
		...
		case 'V':
                {
			glutIdleFunc ((void (* ) (void)) set_vergence_coarse2fine);
		}       
		break;
		case 'v':
		{
