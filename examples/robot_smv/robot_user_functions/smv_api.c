...
DLLEXPORT void maeInitialize ()
{
	...
	init_user_functions ();
	g_pList = listCreate ();
	g_pUndistortedImage = (int *) malloc (INPUT_WIDTH * INPUT_HEIGHT * sizeof (int));
...
DLLEXPORT void maeMapWorldPoints (int *pPointsVectorX, int *pPointsVectorY, int nPointsNumber)
{
	...
	for (i = 0; i < nPointsNumber; i++)
	{
		...
		fprintf (g_pLogFile, "\t(%d, %d) <-> (%d, %d)\n", (int) leftPoint.x, (int) leftPoint.y, (int) rightPoint.x, (int) rightPoint.y);
		fclose (g_pLogFile);
#endif
