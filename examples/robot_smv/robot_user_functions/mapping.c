...
void TMapInitialize(TMap *map, int img_width, int img_height, float lambda)
{
	...
	for (j = 0; j < img_height; j++)
	{
		...
		// Calcula beta
		beta = ATAN((float)((img_height / 2) - j) / lambda);

		// Inicializa os pontos
		for (i = 0; i < img_width; i++)
		{
			map->point[j][i].beta = beta;
			alfa = ATAN((float)((img_width / 2) - i) / lambda);	// Calcula o alfa
...
{
	int i, j;
	float fMinX = .0f, fMaxX = .0f;
	float fMinY = .0f, fMaxY = .0f;
	float fMinZ = .0f, fMaxZ = .0f;
