...
//
//	if (cbAvaiableSpace(sound_buffer) < frames * columns)
//	{
...
//
//	if (space_until_buffer_reentrance < frames * columns)
//	{
...
//	size = 2 * h;
//
//	sound_fft = (fftw_complex*) fftw_malloc ((h + 1) * sizeof(fftw_complex));
//
//	for(j = 0; j < w; j++)
//	{
//		for(i = 0; i < h; i++)
//		{
//			sound_fft[i][0] = output->neuron_layer->neuron_vector[3 * (j + i * w) + 0].output.fval;
//			sound_fft[i][0] = output->neuron_layer->neuron_vector[3 * (j + i * w) + 1].output.fval;
//		}
//
//		compute_inverse_fft (sound_fft, &sound, size);
//		create_playback_sound (sound, &frames, size);
//		playback_sound (frames, 4 * size);
//
//		free (sound);
//		free (frames);
//	}
