...
update_interface()
{
	...
	switch(state.state)
	{
		...
		case WAIT_MAX_TO_SELL:
			...
			sprintf(aux, "%f%%", 100.0 * (state.price - state.buy_price) / state.buy_price);
			fl_set_object_label(current_return_val, aux);
			break;
...
state_handler (MSG_INSTANCE msgRef, void *callData, void *clientData)
{
	...
	if (price_samples == NULL)
		price_samples = (float *) malloc(sizeof(float) * MINUTES_IN_A_WEEK);
