...
control_handler (MSG_INSTANCE msgRef, void *callData, void *clientData)
{
	...
	else  if (received_control->type == 3) // buy/sell order
	{
		...
		{
			sscanf(received_control->description, "B %d %s %f", &(state.quantity), aux, &(state.order_price)); // Ex: B 400 PETR4 43.10  ou B 400 PETR4 43.10
			state.state = SEND_BUY_ORDER;
		}
		else if ((received_control->description[0] == 'S') && (state.state == WAIT_MAX_TO_SELL))
		{
			sscanf(received_control->description, "S %d %s %f", &(state.quantity), aux, &(state.order_price)); // Ex: B 400 PETR4 43.10  ou B 400 PETR4 43.10
