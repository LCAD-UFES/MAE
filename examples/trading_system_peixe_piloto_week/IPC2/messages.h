...
#define QSOR_QUOTES_MESSAGE_FORMAT		"{int,<string:1>,<float:1>,<float:1>,string,string}"
...
#define QSOR_ORDER_EXECUTION_MESSAGE_FORMAT	"{int,string,float,int,string,string}"
...
#define TS_ORDER_MESSAGE_FORMAT			"{int,string,float,int,string,string}"
...
typedef struct 
{
	...
	float *price_samples;	// contains all stored samples of a week; at each minute a sample is added
	char *date_and_time;
	char *host;
} ts_state_message;

#define TS_STATE_MESSAGE_FORMAT			"{int,int,<ubyte: 1>,<float:2>,string,string}"
...
float* aloca_array_float(int array_tam);
char* aloca_string(int string_tam);
void build_array_string(char** array_string, char* file_string);
void build_array_float(float* array_float, char* file_string);
