...
#define NL_TARGET_COORDINATES_COARSE_NAME	"nl_target_coordinates_coarse"
#define NL_TARGET_COORDINATES_SOSO_NAME		"nl_target_coordinates_soso"
#define NL_TARGET_COORDINATES_FINE_NAME		"nl_target_coordinates_fine"

#define NL_FEATURES_COARSE_NAME			"nl_features_coarse"
#define NL_FEATURES_SOSO_NAME			"nl_features_soso"
#define NL_FEATURES_FINE_NAME			"nl_features_fine"

#define NL_V1_ROW_COARSE_NAME			"nl_v1_row_coarse"
#define NL_V1_ROW_SOSO_NAME			"nl_v1_row_soso"
#define NL_V1_ROW_FINE_NAME			"nl_v1_row_fine"
...
void 
jump (INPUT_DESC *input, int step)
{
	static NEURON_LAYER *nl_target_coordinates_coarse = NULL;
	static NEURON_LAYER *nl_target_coordinates_soso = NULL;
	static NEURON_LAYER *nl_target_coordinates_fine = NULL;
	NEURON_LAYER *nl_target_coordinates = NULL;
	int x, y;
	
	// Gets the target coordinates neuron layers
	if (nl_target_coordinates_coarse == (NEURON_LAYER *) NULL)
	{
		nl_target_coordinates_coarse = get_neuron_layer_by_name (NL_TARGET_COORDINATES_COARSE_NAME);
		nl_target_coordinates_soso   = get_neuron_layer_by_name (NL_TARGET_COORDINATES_SOSO_NAME);
		nl_target_coordinates_fine   = get_neuron_layer_by_name (NL_TARGET_COORDINATES_FINE_NAME);
	}
	...
	{
		case COARSE:
			nl_target_coordinates = nl_target_coordinates_coarse;
			break;
		case SOSO:
			nl_target_coordinates = nl_target_coordinates_soso;
			break;
		case FINE:
			nl_target_coordinates = nl_target_coordinates_fine;
			break;
	}
...
void 
saccade (INPUT_DESC *input, int step)
{
	static NEURON_LAYER *nl_target_coordinates_coarse = NULL;
	static NEURON_LAYER *nl_target_coordinates_soso = NULL;
	static NEURON_LAYER *nl_target_coordinates_fine = NULL;
...
/*
********************************************************
* Function: memorize_features_vector		       *
* Description: 					       *
* Inputs: 					       *
* Output: 	  				       *
********************************************************
...
void 
memorize_features_vector ()
{
	static NEURON_LAYER *nl_features_coarse = NULL;
	static NEURON_LAYER *nl_features_soso   = NULL;
	static NEURON_LAYER *nl_features_fine   = NULL;
	static NEURON_LAYER *nl_v1_row_coarse   = NULL;
	static NEURON_LAYER *nl_v1_row_soso     = NULL;
	static NEURON_LAYER *nl_v1_row_fine     = NULL;
	static int index = 0;
	int i;
	
	if (nl_features_coarse == NULL)
	{
		nl_features_coarse = get_neuron_layer_by_name (NL_FEATURES_COARSE_NAME);
		nl_features_soso   = get_neuron_layer_by_name (NL_FEATURES_SOSO_NAME);
		nl_features_fine   = get_neuron_layer_by_name (NL_FEATURES_FINE_NAME);
		nl_v1_row_coarse   = get_neuron_layer_by_name (NL_V1_ROW_COARSE_NAME);
		nl_v1_row_soso     = get_neuron_layer_by_name (NL_V1_ROW_SOSO_NAME);
		nl_v1_row_fine     = get_neuron_layer_by_name (NL_V1_ROW_FINE_NAME);					       
	}
	
	for (i = 0; i < nl_v1_row_coarse->dimentions.x; i++)
		nl_v1_row_coarse->neuron_vector[i + index * nl_v1_row_coarse->dimentions.x] = nl_features_coarse->neuron_vector[i];
	
	for (i = 0; i < nl_v1_row_soso->dimentions.x; i++)
		nl_v1_row_soso->neuron_vector[i + index * nl_v1_row_soso->dimentions.x] = nl_features_soso->neuron_vector[i];

	for (i = 0; i < nl_v1_row_fine->dimentions.x; i++)
		nl_v1_row_fine->neuron_vector[i + index * nl_v1_row_fine->dimentions.x] = nl_features_fine->neuron_vector[i];

	index = (index + 1) % nl_v1_row_coarse->dimentions.y;
}
...
f_keyboard (char *key_value)
{
	...
	switch (key = key_value[0])
	{
		...
		case 't':
			...
			run_network (1);
			break;
		// Memorizes the Current Features Vector
		case 'M':	
		case 'm':
			memorize_features_vector ();
			break;
		...
		case 'e':
			samplesNumber++;
			matchsNumber += match = sqrtf ((g_nNoseX - inPattern->wxd) * (g_nNoseX - inPattern->wxd) + (g_nNoseY - inPattern->wyd) * (g_nNoseY - inPattern->wyd)) <= 5.0 ? 1 : 0;
		...
		case 'n':
			GetNewFace (inPattern, DIRECTION_FORWARD);
			break;
		// Saccade until reach the target (coarse step)
		case 'C':
			saccade (inPattern, COARSE);
		// Saccade until reach the target (more or less step)
		case 'S':
			saccade (inPattern, SOSO);		
		// Saccade until reach the target (fine step)
		case 'F':
			saccade (inPattern, FINE);
		// Jump to the target (coarse step)
		case 'c':
			jump (inPattern, COARSE);
			break;
		// Jump to the target (more or less step)
		case 's':
			jump (inPattern, SOSO);
			break;
		// Jump to the target (fine step)
		case 'f':
			jump (inPattern, FINE);
			break;		
	}
