...
void set_vergence (TPoint *point)
{
	...
	if (g_DispData == NULL)
	{
		...
		g_AllDispDataProp = (DISP_DATA *) alloc_mem (sizeof (DISP_DATA));
		g_AllDispDataProp->samples = (SAMPLES **) alloc_mem (sizeof (SAMPLES *) * nAllSamples);
		for (i = 0; i < nAllSamples; i++)
			g_AllDispDataProp->samples[i] = (SAMPLES *) alloc_mem (sizeof (SAMPLES) * (wo * ho));
	}
	...
	for (image_left.wxd = nMinScan; image_left.wxd < nMaxScan; image_left.wxd++)
	{
		...
		// plot MT_CELL
		sprintf(strAux, "%d %.3f\n", image_left.wxd, nl_mt_gaussian.neuron_vector[INDEX_MT_CELL].output.fval);
		StrReplaceChar(strAux, ',', '.');
		fprintf (file_mt_cell, "%s", strAux);

		// plot VERGENCE
		sprintf(strAux, "%d %.3f\n", image_left.wxd, fltSumOutputCells);
		StrReplaceChar(strAux, ',', '.');
...
float evidence_propagation (int i, int j, int k, int w, int h, int kmax, float fltSmoothness, float fltPMin)
{
	float I;
	...
	I       = g_AllDispData->samples[k][j * w + i].val;

	// P is the confidence value of point (i,j) at one specific disparity layer
	P       = g_AllDispData->samples[k][j * w + i].conf;
	P_up    = (j ==  0         ? 0.0 : g_AllDispData->samples[k][(j - 1) * w + i].conf);
	P_down  = (j == (h - 1)    ? 0.0 : g_AllDispData->samples[k][(j + 1) * w + i].conf);
	P_left  = (i ==  0         ? 0.0 : g_AllDispData->samples[k][j * w + (i - 1)].conf);
	P_right = (i == (w - 1)    ? 0.0 : g_AllDispData->samples[k][j * w + (i + 1)].conf);
	P_low   = (k ==  0         ? 0.0 : g_AllDispData->samples[k - 1][j * w + i].conf);
	P_high  = (k == (kmax - 1) ? 0.0 : g_AllDispData->samples[k + 1][j * w + i].conf);

	// L is the modified I value after the propagation of neighbors evidences
	L_up    = (j ==  0         ? 0.0 : g_AllDispDataProp->samples[k][(j - 1) * w + i].val);
	L_down  = (j == (h - 1)    ? 0.0 : g_AllDispDataProp->samples[k][(j + 1) * w + i].val);
	L_left  = (i ==  0         ? 0.0 : g_AllDispDataProp->samples[k][j * w + (i - 1)].val);
	L_right = (i == (w - 1)    ? 0.0 : g_AllDispDataProp->samples[k][j * w + (i + 1)].val);
	L_low   = (k ==  0         ? 0.0 : g_AllDispDataProp->samples[k - 1][j * w + i].val);
	L_high  = (k == (kmax - 1) ? 0.0 : g_AllDispDataProp->samples[k + 1][j * w + i].val);
	...
	W_low   = (k ==  0         ? 0.0 : permeability_weight (P_low,   P, fltPMin));
	W_high  = (k == (kmax - 1) ? 0.0 : permeability_weight (P_high,  P, fltPMin));

	weighted_sum = (I + fltSmoothness * (L_up * W_up + L_down * W_down + L_left * W_left + L_right * W_right + L_low * W_low + L_high * W_high));
...
void copy_AllDispData (DISP_DATA *dest, DISP_DATA *source, int nSamples, int nNeurons)
{
	int i, k;
	...
	for (k = 0; k < nSamples; k++)
	{
		...
		{
			dest->samples[k][i] = source->samples[k][i];
		}
	}
}
...
void evidence_propagation_all ( )
{
	int i, j, k, n, nIndex;
	int w, h, kmax;
	float fltSmoothness, fltPMin, fltOutput, fltSum, previous_avg_val, dif_val; 
	static float avg_val = FLT_MAX;

	if (g_AllDispDataProp == NULL)
	{
		printf ("Must execute vergence prior to evidence propagation\n");
		glutIdleFunc ((void (* ) (void))fl_check_forms);
		return;
	}
	...
	w = nl_mt_gaussian.dimentions.x;
	h = nl_mt_gaussian.dimentions.y;
	kmax = nAllSamples;

	copy_AllDispData (g_AllDispDataProp, g_AllDispData, kmax, w * h);
	...
	for (n = 0; n < MAX_ITERATIONS; n++)
	{
		...
		for (k = 0; k < kmax; k++)
		{
			for (i = 0; i < w; i++)
			{
				for (j = 0; j < h; j++)
				{
					fltOutput = evidence_propagation (i, j, k, w, h, kmax, fltSmoothness, fltPMin);
					g_AllDispDataProp->samples[k][j * w + i].val = fltOutput;
					avg_val += fltOutput; 
				}
			}
		}
	...
	copy_AllDispData (g_AllDispData, g_AllDispDataProp, kmax, w * h);

	// Inicializar a estrutura DISP_DATA
	InitDispDataState (w * h);
	...
	for (k = 0; k < kmax; k++)
	{
		for (i = 0; i < w; i++)
		{
			for (j = 0; j < h; j++)
			{
				...
				if (g_AllDispData->samples[k][nIndex].val >= g_fltDispAux[nIndex])
				{
					if (g_DispData->neg_slope[nIndex])
					{
						g_DispData->neg_slope[nIndex] = 0;
						AddLocalMin (nIndex, g_fltDispAux[nIndex], g_fltConfAux[nIndex], g_AllDispData->samples[k - 1][nIndex].pos);
					}
				}
				else
					g_DispData->neg_slope[nIndex] = 1;

				g_fltDispAux[nIndex] = g_AllDispData->samples[k][nIndex].val;
				g_fltConfAux[nIndex] = g_AllDispData->samples[k][nIndex].conf;
			}
		}
	}

	// Considera o ultimo minimo local
	for (i = 0; i < (w * h); i++)
		if (g_DispData->neg_slope[i])
			AddLocalMin (i, g_fltDispAux[i], g_fltConfAux[i], g_AllDispData->samples[k - 1][i].pos);

	// Escolhe a menor disparidade
	for (i = 0; i < (w * h); i++)
	{
		nl_mt_gaussian_map.neuron_vector[i].output.fval = g_DispData->samples[0][i].val;
		nl_confidence_map.neuron_vector[i].output.fval = g_DispData->samples[0][i].conf;
		nl_disparity_map.neuron_vector[i].output.fval = (float) (g_DispData->samples[0][i].pos - image_left.wxd);
	}
	...
	for (i = 0; i < (w * h); i++)
	{
		fltSum = 0.0;
		for (k = 0; k < kmax; k++) 
			fltSum += g_AllDispData->samples[k][i].val;
		nl_mt_gaussian_var.neuron_vector[i].output.fval = (fltSum / kmax) - g_DispData->samples[0][i].val;
	}
...
void f_keyboard (char *key_value)
{
	...
	switch (key)
	{
		...
		case 'h':
		{
			...
			printf ("v : Set vergence on left image based on right image focus and calculate maps:\n    disparity, confidence, MT gaussian, MT gaussian var and AllDispData\n");
			...
			printf ("a : Save world map\n");
			printf ("q : Load world map\n");
			...
			printf ("i : Pass illuminance and reflectance filters until convergence\n");
			printf ("\n");
		}
		break;
