...
{	
	...
	if (filter_desc->private_state == NULL)
	{
		receptive_field = (RECEPTIVE_FIELD_DESCRIPTION *) malloc (ho * sizeof(RECEPTIVE_FIELD_DESCRIPTION));

		// Calculates the spatial frequency (cycles per pixel)
		frequency = (1.0 / (float) wi) * camera_opening * cycles_per_degree;

		for (yo = 0; yo < ho; yo++)
			compute_v1_spatial_receptive_field (&(receptive_field[yo]), frequency, width, aspect_ratio, orientation + pi * (0.5 - (float) yo / (float) ho), phase);
	...
	for (xo = 0; xo < wo; xo++)
	{
		for (yo = 0; yo < ho; yo++)
		{
			index = (xo < wo / 2) ? ho - yo - 1 : yo;
//			previous_output = filter_desc->output->neuron_vector[(yo * wo) + xo].output = bidimentional_cortex_convolution (&(receptive_field[index]), image_n_l, xo, yo, wo, ho);
			previous_output = filter_desc->output->neuron_vector[(yo * wo) + xo].output = bidimentional_convolution (&(receptive_field[index]), image_n_l, xo, yo, wi, hi);
