...
control_handler (MSG_INSTANCE msgRef, void *callData, void *clientData)
{
	...
	{
		state = get_state();
		if ((received_control->description[0] == 'B') && ((state.state == WAIT_MOVING_AVERAGE) || (state.state == WAIT_MIN_TO_BUY)))
		{
			sscanf(received_control->description, "B %d %s %lf", &(state.quantity), aux, &(state.order_price)); // Ex: B 400 PETR4 43.10  ou B 400 PETR4 43.10
			state.state = SEND_BUY_ORDER;
		}
		else if ((received_control->description[0] == 'S') && (state.state == WAIT_MAX_TO_SELL))
		{
			sscanf(received_control->description, "S %d %s %lf", &(state.quantity), aux, &(state.order_price)); // Ex: B 400 PETR4 43.10  ou B 400 PETR4 43.10
