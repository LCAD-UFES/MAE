...
double g_train_return_average = 1.0;
double g_train_return_standard_deviation = 1.0;
...
void 
EvaluateOutputAverage (OUTPUT_DESC *output, double *buy_sell_return, double *sell_buy_return, double *confidence, int *best_day_index)
{
	int i, j;
	...
	*buy_sell_return = *sell_buy_return = 0.0;
	...
	for (i = 0; i < num_neurons; i++)
	{
		...
		*buy_sell_return += g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * current_week].neuron_return_buy_sell;
		*sell_buy_return += g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * current_week].neuron_return_sell_buy;
	...
	*confidence = (double) nMax / (double) num_neurons;
	*best_day_index = FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * selected_week;

	*buy_sell_return = *buy_sell_return / (double) num_neurons;
	*sell_buy_return = *sell_buy_return / (double) num_neurons;
}
...
void 
EvaluateOutput(OUTPUT_DESC *output, double *buy_sell_return, double *sell_buy_return, double *confidence, int *best_day_index)
{
	int i, j;
	int current_week, selected_week = 0;
	int nAux, nMax = 0;
	...
	*best_day_index = FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * selected_week;
	...
	printf ("#|%s-%s - > % .6f\n", Tdate, Tdow, g_train_data_set[*best_day_index].neuron_return_buy_sell);
	
#endif
	*buy_sell_return = g_train_data_set[*best_day_index].neuron_return_buy_sell;
	*sell_buy_return = g_train_data_set[*best_day_index].neuron_return_sell_buy;
}



void 
mean_spread_predictor(OUTPUT_DESC *output, double *buy_sell_return, double *sell_buy_return, double *confidence, int *best_day_index)
{
	int i, index;
	int begin_date;
	double ref_price, stock_base_price, average_spread;
#define	COMPUTE_INDEX(base,displacement)((base) + (displacement) * NUM_DAY_IN_A_WEEK)

	begin_date = FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber - (SAMPLE_SIZE + 2) * NUM_DAY_IN_A_WEEK;
	stock_base_price = g_train_data_set[begin_date].clo;
	ref_price = g_ref_data_set[begin_date].clo;

	average_spread = 0.0;
	for (i = 0; i < SAMPLE_SIZE; i++)
	{
		index = COMPUTE_INDEX(begin_date,i);
		average_spread += (g_train_data_set[index].clo / stock_base_price) - (g_ref_data_set[index].clo / ref_price);
	}
	average_spread /= (double) SAMPLE_SIZE;
	index = COMPUTE_INDEX(begin_date,i);
	*buy_sell_return = ((g_train_data_set[index].clo / stock_base_price) - (g_ref_data_set[index].clo / ref_price) - average_spread) / 2.0;

	average_spread = 0.0;
	for (i = 0; i < SAMPLE_SIZE; i++)
	{
		index = COMPUTE_INDEX(begin_date,i);
		average_spread += (g_ref_data_set[index].clo / ref_price) - (g_train_data_set[index].clo / stock_base_price);
	}
	average_spread /= (double) SAMPLE_SIZE;
	index = COMPUTE_INDEX(begin_date,i);
	*sell_buy_return = ((g_ref_data_set[index].clo / ref_price) - (g_train_data_set[index].clo / stock_base_price) - average_spread) / 2.0;

	*confidence = 0.5;
	*best_day_index = FIRST_SAMPLE;

#if VERBOSE == 1
	// printf("week id = %d\n", selected_week);
	char Tdate[100], Tdow[100]; // Tdata = target date
	
	strcpy (Tdate, long2isodate (g_train_data_set[index].date_l));
	strcpy (Tdow, long2dows (g_train_data_set[index].date_l));
	printf ("#|%s-%s - > % .6f\n", Tdate, Tdow, *buy_sell_return);
...
void
base_output_handler (OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state, void (*EvaluateOutput) (OUTPUT_DESC *, double *, double *, double *, int *))
{
	...
	if (g_nStatus == RECALL_PHASE)
	{
		...
		//(*EvaluateOutput) (output, &wnn_predicted_return_buy_sell, &wnn_predicted_return_sell_buy, &confidence, &best_day_index);
		mean_spread_predictor(output, &wnn_predicted_return_buy_sell, &wnn_predicted_return_sell_buy, &confidence, &best_day_index);
		...
		if ((FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber) <= LAST_SAMPLE)
		{
			...
			g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_predicted_return_buy_sell = wnn_predicted_return_buy_sell;
			g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_prediction_error_buy_sell = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].week_return_buy_sell - wnn_predicted_return_buy_sell;

			g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_predicted_return_sell_buy = wnn_predicted_return_sell_buy;
			g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_prediction_error_sell_buy = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].week_return_sell_buy - wnn_predicted_return_sell_buy;

			g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].confidence = confidence;

			printf ("%s %s ; % .6lf  ; % .6lf ; % .6lf  ;    % .6lf ; % .6lf ; % .6lf  ;    %0.3lf ;\n", 
				 ...
				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_predicted_return_buy_sell,
				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_prediction_error_buy_sell,

				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].week_return_sell_buy,
				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_predicted_return_sell_buy,
				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].wnn_prediction_error_sell_buy,
				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].confidence);
		}
		else
		{
			printf ("%s %s ;            ; % .6lf ;            ;              ; % .6lf ;            ;    %0.3lf ;\n", 
				 ...
				 wnn_predicted_return_buy_sell,
				 wnn_predicted_return_sell_buy,
				 g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].confidence);
		}		
	
#if	RELEARN_RETURN == 1
		g_train_data_set[best_day_index].neuron_return_buy_sell = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].week_return_buy_sell;
		g_train_data_set[best_day_index].neuron_return_sell_buy = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * g_TargetWeekNumber].week_return_sell_buy;
#endif
	}
...
show_statistics_buy_sell()
{
	...
	N = 0;
	for (i = g_first_tested; i < g_first_tested + g_nTotalTested; i++)
	{
		if (g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].confidence > 0.0) // 0.6)
		{
			tgt = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].week_return_buy_sell;
			out = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].wnn_predicted_return_buy_sell;
			err = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].wnn_prediction_error_buy_sell;
	...
	printf ("# target date  ; tgt. ret.  ; pred. ret.; pred. error;   tgt. ret.  ; pred. ret.; pred. error;confidence; %s\n", g_current_data_set_name);
	printf ("#  ****  average_target_return_buy_sell: % .6lf, average_wnn_error_buy_sell: % .6lf\n",
			  average_target_return,
		          average_wnn_prediction_error);

	printf ("#  ****  standard_deviation_target_return_buy_sell: % .6lf, standard_deviation_wnn_error_buy_sell: % .6lf\n",
			  standard_deviation_target_return,
		          standard_deviation_wnn_error);

	printf ("#  ****  hr_up_buy_sell: %.2lf, hr_dn_buy_sell: %.2lf, hr_buy_sell: %.2lf\n",  100.0 * hr_up, 100.0 * hr_dn, 100.0 * hr);
}



void
show_statistics_sell_buy()
{
	int N, i;
	double average_wnn_prediction_error = 0.0;
	double standard_deviation_wnn_error = 0.0;
	...
	{
		tgt = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].week_return_sell_buy;
		out = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].wnn_predicted_return_sell_buy;
		err = g_train_data_set[FIRST_SAMPLE + NUM_DAY_IN_A_WEEK * i].wnn_prediction_error_sell_buy;
	...
	printf ("#\n");
	printf ("#  ****  average_target_return_sell_buy: % .6lf, average_wnn_error_sell_buy: % .6lf\n",
			  average_target_return,
		          average_wnn_prediction_error);

	printf ("#  ****  standard_deviation_target_return_sell_buy: % .6lf, standard_deviation_wnn_error_sell_buy: % .6lf\n",
			  standard_deviation_target_return,
		          standard_deviation_wnn_error);

	printf ("#  ****  hr_up_sell_buy: %.2lf, hr_dn_sell_buy: %.2lf, hr_sell_buy: %.2lf\n",  100.0 * hr_up, 100.0 * hr_dn, 100.0 * hr);
}
