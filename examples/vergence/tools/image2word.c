...
main(int argc, char **argv)
{
	...
	if ((argc != 3) && (argc != 5))
	{
		...
		printf ("./convert <disparity_map_file> <color_map_file> <focal_distance> <camera_distance>\n");
	...
	if (argc == 5)
	{
		...
		CAMERA_DISTANCE = atof (argv[4]);
	}
	
	/* Get the disparity_map dimentions */
	fscanf(disparity_map_file, "%d %d\n", &(w), &(h));
	
	disparity_map = (int *) malloc (w * h * sizeof (int));
	
	for (i = 0; i < (w * h); i++)
		fscanf(disparity_map_file, "%d", &(disparity_map[i]));
	...
	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			...
			map_v1_to_image (&(xi), &(yi), wi, hi, u, v, w, h, 238, 189);
			...
			right_point.x = (double) (238 + xi);
			right_point.y = (double) (189 + yi);

			left_point.x = (double) (290 + xi + disparity);
			left_point.y = (double) (189 + yi);
