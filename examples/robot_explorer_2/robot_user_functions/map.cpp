...
TMap::TMap (int nWidth, int nHeight, float fltBetaMin, float fltBetaMax, int nLayersNumber, int nEspheresNumber, float fltNear, float fltFar, float fltTerrainSize)
{
	int i, j;
	...
	espheresNumber = nEspheresNumber;
	imagePlaneTex = 0;
	...
	*/
       ...
       // Allocates memory for the grids vector
        if ((grid = new TPoint **[espheresNumber]) == NULL)
		Error ("Cannot allocate memory to map grid (TMap::TMap)", "", "");

	for (int j=0; j<espheresNumber; j++)
	{
		if ((grid[j] = new TPoint *[layersNumber]) == NULL)
			Error ("Cannot allocate memory to map grid (TMap::TMap)", "", "");

		if (( *grid[j] = new TPoint[layersNumber * width * height]) == NULL)
			Error ("Cannot allocate memory to map grid (TMap::TMap)", "", "");
	
        	for (i = 0; i < layersNumber; i++)
		{
               		//*(grid[j])[i] = &(*(grid[j])[0][i * width * height]);
			TPoint **aux = grid[j];
			aux[i] = & (aux[0][i * width * height]);
		}
	}
	...
	*/          
      ...
      // Allocates memory for the images vector
        if ((image = new GLubyte **[espheresNumber]) == NULL)
		Error ("Cannot allocate memory to map image (TMap::TMap)", "", "");

	for (int j=0; j<espheresNumber; j++)
	{
		if ((image[j] = new GLubyte *[layersNumber]) == NULL)
			Error ("Cannot allocate memory to map image (TMap::TMap)", "", "");

		if (( *image[j] = new GLubyte[layersNumber * 3 * width * height]) == NULL)
			Error ("Cannot allocate memory to map image (TMap::TMap)", "", "");
	
        	for (i = 0; i < layersNumber; i++)
		{
               		//*(image[j])[i] = &(*(image[j])[0][i * width * height]);
			GLubyte **aux = image[j];
			aux[i] = & (aux[0][i * 3 * width * height]);
		}
	}
	...
	*/
      ...
      // Allocates memory for the depth map vector
        if ((depthMap = new float **[espheresNumber]) == NULL)
		Error ("Cannot allocate memory to map depth (TMap::TMap)", "", "");

	for (int j=0; j<espheresNumber; j++)
	{
		if ((depthMap[j] = new float *[layersNumber]) == NULL)
			Error ("Cannot allocate memory to map depth (TMap::TMap)", "", "");

		if (( *depthMap[j] = new float[layersNumber * width * height]) == NULL)
			Error ("Cannot allocate memory to map depth (TMap::TMap)", "", "");
	
        	for (i = 0; i < layersNumber; i++)
		{
        
			float **aux = depthMap[j];
			aux[i] = & (aux[0][i * width * height]);
		}
	}
	...
	for (j = 0; j < height; j++)
	{
                ...
                {
			// Completar
                        depthMap[0][currentLayer][i + j * width] = UNKNOWN;
			image[0][currentLayer][3 * (i + j * width) + 0] = image[0][currentLayer][3 * (i + j * width) + 1] = image[0][currentLayer][3 * (i + j * width) + 2] = 0;
...
bool TMap::Load (char *strFileName)
{
	...
	*/
	...
	// Reads the map grid
	// Completar
        if (fread (grid[0][0], sizeof (TPoint), layersNumber * width * height, pFile) != (unsigned int) (layersNumber * width * height))
        {
	...
	*/
	...
	// Reads the map image
	// Completar
        if (fread (image[0][0], sizeof (GLubyte), 3 * width * height, pFile) != (unsigned int)(layersNumber * 3 * width * height))
        {
                Error ("While reading file '", strFileName, "' (TMap::Load)");
                return (false);
        }
...
bool TMap::Save (char *strFileName)
{
	...
	// Completar
        if (fwrite (grid[0][0], sizeof (TPoint), layersNumber * width * height, pFile) != (unsigned int)(layersNumber * width * height))
        {
                Error ("While writing file '", strFileName, "' (TMap::Save)");
                return (false);
        }
	...
	// Completar
        if (fwrite (image[0][0], sizeof (GLubyte), layersNumber * 3 * width * height, pFile) != (unsigned int)(layersNumber * 3 * width * height))
        {
                Error ("While writing file '", strFileName, "' (TMap::Save)");
                return (false);
        }
...
bool TMap::SaveImage (char *strFileName)
{
	...
	for (j = 0; j < height; j++)
		for (i = 0; i < width; i++)
			// Completar
			fprintf (pFile, "%d %d %d\n",
                        image[0][currentLayer][3 * (i + (height - 1 - j) * width) + 0],
                        image[0][currentLayer][3 * (i + (height - 1 - j) * width) + 1],
                        image[0][currentLayer][3 * (i + (height - 1 - j) * width) + 2]);
...
void TMap::Update (float X, float Y, float Z, GLubyte red, GLubyte green, GLubyte blue)
{
	...
	// Completar
        depthMap[0][currentLayer][i + j * width] = fltDistance;
        
	// Completar 
        grid[0][currentLayer][i + j * width].x = X;
        grid[0][currentLayer][i + j * width].y = Y;
        grid[0][currentLayer][i + j * width].z = Z;
                
	// Completar          
        image[0][currentLayer][3 * (i + j * width) + 0] = red;
        image[0][currentLayer][3 * (i + j * width) + 1] = green;
        image[0][currentLayer][3 * (i + j * width) + 2] = blue;
...
void TMap::Erase (void)
{
	...
	{
		// Completar
        	depthMap[0][currentLayer][i] = UNKNOWN;
        	image[0][currentLayer][3 * i + 0] = image[0][currentLayer][3 * i + 1] = image[0][currentLayer][3 * i + 2] = 0;
	}
...
void TMap::Rotate (float fltDeltaAlfa)
{
	...
	for (i = 0; i < width * height; i++)
        {
		// Completar
                depthMap[0][nNextLayer][i] = UNKNOWN;
                image[0][nNextLayer][3 * i + 0] = image[0][nNextLayer][3 * i + 1] = image[0][nNextLayer][3 * i + 2] = 0;
        }
	...
	for (j = 0; j < height; j++)
	{
                ...
                {
			// Completar  
                        if (depthMap[0][currentLayer][i + j * width] == UNKNOWN)
				continue;
			...
			ConvertRect2Sphe (&fltAlpha, 
					  &fltBeta, 
					  &fltDistance, 
					  grid[0][currentLayer][i + j * width].x, 
					  grid[0][currentLayer][i + j * width].y, 
					  grid[0][currentLayer][i + j * width].z);
			
			io = ((io = i - (int) fltDelta) >= 0) ? io % width : width + io;
				
			// Completar
                        depthMap[0][nNextLayer][io + j * width] = fltDistance;
                        
			// Completar
                        image[0][nNextLayer][3 * (io + j * width) + 0] = image[0][currentLayer][3 * (i + j * width) + 0];
                        image[0][nNextLayer][3 * (io + j * width) + 1] = image[0][currentLayer][3 * (i + j * width) + 1];
                        image[0][nNextLayer][3 * (io + j * width) + 2] = image[0][currentLayer][3 * (i + j * width) + 2];

			// Completar
                        ConvertSphe2Rect (&grid[0][nNextLayer][io + j * width].x,
                                          &grid[0][nNextLayer][io + j * width].y,
                                          &grid[0][nNextLayer][io + j * width].z,
...
void TMap::Forward (float fltStep)
{
        ...
        for (j = 0; j < height; j++)
        {
			...
			// Completar
					  ...
					  &fltNewBeta, 
					  &fltNewDistance, 
					  grid[0][currentLayer][i + j * width].x + fltStep, 
					  grid[0][currentLayer][i + j * width].y, 
					  grid[0][currentLayer][i + j * width].z);
			...
			// Completar
                        depthMap[0][nNextLayer][io + jo * width] = fltNewDistance;
			
                        image[0][nNextLayer][3 * (io + jo * width) + 0] = image[0][currentLayer][3 * (i + j * width) + 0];
                        image[0][nNextLayer][3 * (io + jo * width) + 1] = image[0][currentLayer][3 * (i + j * width) + 1];
                        image[0][nNextLayer][3 * (io + jo * width) + 2] = image[0][currentLayer][3 * (i + j * width) + 2];

			// Completar
                        grid[0][nNextLayer][io + jo * width].x = grid[0][currentLayer][i + j * width].x + fltStep;
                        grid[0][nNextLayer][io + jo * width].y = grid[0][currentLayer][i + j * width].y;
                        grid[0][nNextLayer][io + jo * width].z = grid[0][currentLayer][i + j * width].z;
                }
        }
...
void TMap::Translate (float fltStepX, float fltStepY)
{
        ...
        for (j = 0; j < height; j++)
        {
			...
			// Completar
					  ...
					  &fltNewBeta, 
					  &fltNewDistance, 
					  grid[0][currentLayer][i + j * width].x + fltStepX, 
					  grid[0][currentLayer][i + j * width].y + fltStepY, 
					  grid[0][currentLayer][i + j * width].z);
			...
			// Completar
                        grid[0][nNextLayer][io + jo * width].x = grid[0][currentLayer][i + j * width].x + fltStepX;
                        grid[0][nNextLayer][io + jo * width].y = grid[0][currentLayer][i + j * width].y + fltStepY;
                        grid[0][nNextLayer][io + jo * width].z = grid[0][currentLayer][i + j * width].z;
                }
        }
...
void TMap::Draw (void)
{
	...
	{
		// Completar
                if (depthMap[0][currentLayer][i] != UNKNOWN)
                {
			// Completar
                        glColor3ub (image[0][currentLayer][3 * i + 0], image[0][currentLayer][3 * i + 1], image[0][currentLayer][3 * i + 2]);
                        glVertex3f (grid[0][currentLayer][i].x, grid[0][currentLayer][i].y, grid[0][currentLayer][i].z);
                }
        }
        glEnd ();
...
void TMap::ShowImage (void)
{
	...
	// Completar
	glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, (void *) image[0][currentLayer]);
	glBegin (GL_QUADS);
...
void TMap::ShowDepthMap (void)
{
	...
	// Completar
	fltFixationPointDepth = depthMap[0][currentLayer][width / 2 + height / 2 * width];
	
	// Completar
        for (i = 0, fltDepth = depthMap[0][currentLayer][i]; i < width * height; i++, fltDepth = depthMap[0][currentLayer][i])
        {
		if (fltDepth == UNKNOWN)
		{
...
void TMap::GetFuzzyInputs (double *p_dblFuzzyInputs, int nInputsNumber, float fltMinAlpha, float fltMaxAlpha, float fltFloor)
{
	...
	for (j = 0; j < height; j++)
	{
		for (i = nStartX; i < nEndX; i++)
		{
			...
			// Completar
			fltDepth = depthMap[0][currentLayer][i + j * width];

			// Completar
			if ((fltDepth == UNKNOWN) || (grid[0][currentLayer][i + j * width].z < fltFloor))
				continue;
...
void TMap::GenerateOccupancyGrid (void)
{
	...
	for (i = 0; i < gridWidth * gridLength; i++)
	{
		// Completar
		if (depthMap[0][currentLayer][i] != UNKNOWN)
			UpdateOccupancyGrid (grid[0][currentLayer][i].x, grid[0][currentLayer][i].y, grid[0][currentLayer][i].z);
	}
}
