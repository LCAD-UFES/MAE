...
#define MIN_CONFIANCE_TO_ACCEPT_RETRAIN 400
#define MIN_CONFIANCE_TO_RETRAIN 600
#define NUM_IMAGES_BEFORE_TO_RETRAIN 3
#define NUM_PIXELS 3
#define	sigma 10.0
#define NUM_PIXELS_WEIGHT 5
...
move_input(INPUT_DESC *input)
{
	...
	//	forward_objects("nl_v1_activation_map_neuron_weight_filtered_threshold_filter");
		forward_objects("nl_movement_diff_map_filter");
...
{
	double count, confidence;
	double count_bg, confidence_bg;
	int u, v, value_b, value_r, value_g, value_t_b, value_t_r, value_t_g;
	float altura, largura, dist_b, dist_r, dist_g, raio, raio_2, dist;
		...
		for (u = 0; u < w; u++)
		{
			if ( BLUE(table_v1.neuron_vector[v * w + u].output.ival) > 0 ){
				...
				if (value_b > 0)
				{
					...
					dist += 1.0 / (1.0 + 0.0000001*(dist_r + dist_g)*(dist_r + dist_g));
					
/*					if (dist_r + dist_g == 0.0) 
					{
						confidence += 100.0;
						count += 99.0;
					}
...
*/				}
			...
			else if ( RED(table_v1.neuron_vector[v * w + u].output.ival) == 255 ){
				value_b = BLUE(n[v * w + u].output.ival);
				if (value_b == 0)
				{
					confidence_bg += 100000.0;
				}							
				count_bg += 100000.0;
			}
		}
	...
	//dist = 1.0 / (1.0 + 0.000001*(dist));
	//confidence = (confidence / count);
	confidence = dist / count;
	confidence_bg = pow(confidence_bg/count_bg, 10);
	return confidence * confidence_bg;
}
...
{
    double partial_weight, partial_weight_center, partial_weight_border, partial_weight_border_count;
...
double altura, largura;
    ...
    partial_weight_border_count = partial_weight_border = partial_weight = 0.0;
       ...
       if ( (v_neuron > NUM_PIXELS_WEIGHT - 1) && (v_neuron < h - NUM_PIXELS_WEIGHT))
	{
		...
		{   
		    for (u_neighbor_neuron = lim_inf_u_neighbor; u_neighbor_neuron <= lim_sup_u_neighbor; u_neighbor_neuron++)
		    {
		        ...
		        {
		            neighbor_neuron_output = nl_v1_activation_map.neuron_vector[v_neighbor_neuron * w + u_neighbor_neuron].output.ival;
		            map_v1_to_image(&xi_neighbor, &yi_neighbor, IMAGE_WIDTH_RESIZED, IMAGE_HEIGHT_RESIZED, u_neighbor_neuron, v_neighbor_neuron, w, h, IMAGE_WIDTH_RESIZED/2.0, IMAGE_HEIGHT_RESIZED/2.0, 0, log_factor);			
				...
				if ( BLUE( table.neuron_vector[(yi_neighbor + yi_neuron) * IMAGE_WIDTH_RESIZED + (xi_neighbor + xi_neuron)].output.ival ) == 0 )
				{
				        partial_weight_border_count += 1.0;
				}
		            }
		            else
		            {
...
//    partial_weight = partial_weight / 8.0;
//printf("p %d\n", (yi_neighbor + yi_neuron));
    return partial_weight  + partial_weight_border; //* (partial_weight_border_count > 0.0 ? 1.0 : 0.0);
}
...
{
	int u, v, xi, yi;
	double xi_weight, yi_weight, total_weight, total_weight_conf, total_weight_conf_thresholded, partial_weight, partial_weight_thresholded;
	double xi_weight_temp, yi_weight_temp;
	int x, y;
	int neuron_output;
	int value_b, value_r, value_g, value_t_b, value_t_r, value_t_g;
	double dist, dist_count, dist_bg;

	xi_weight = 0.0;
	yi_weight = 0.0;
	total_weight_conf_thresholded = total_weight = total_weight_conf = 0.0;
	g_xi_micro_weight = 0.0;
	g_yi_micro_weight = 0.0;
	dist_count = dist_bg = dist = 0.0;
	...
	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			value_b = BLUE(nl_v1_activation_map.neuron_vector[v * w + u].output.ival);
			value_t_b = BLUE(table_v1.neuron_vector[v * w + u].output.ival);
			if (value_b > 0)
			{
				...
				partial_weight_thresholded = nl_v1_activation_map_neuron_weight_thresholded.neuron_vector[v * w + u].output.fval;
				partial_weight = nl_v1_activation_map_neuron_weight.neuron_vector[v * w + u].output.fval;

				xi_weight_temp = (double) ((double)(xi - x) * partial_weight_thresholded); 
				yi_weight_temp = (double) ((double)(yi - y) * partial_weight_thresholded);
				xi_weight += xi_weight_temp;
				yi_weight += yi_weight_temp;
				total_weight += (double) partial_weight_thresholded;

				if (value_t_b > 0 )
				{

					dist += sqrt(xi_weight_temp*xi_weight_temp + yi_weight_temp*yi_weight_temp); 
					dist_count += 1.0;
					total_weight_conf += partial_weight;
					total_weight_conf_thresholded += partial_weight_thresholded;
					
					//microsacada
					g_xi_micro_weight += (double) ((double)(xi - x) * partial_weight_thresholded);
					g_yi_micro_weight += (double) ((double)(yi - y) * partial_weight_thresholded);
...
//	printf("total_weight_conf = %f\n", total_weight_conf);
	...
	dist /= dist_count; 
	g_confidence = exp(-0.00001*dist*dist);
	g_confidence *= exp(-0.00001*dist_bg*dist_bg);
	g_confidence = sqrt(g_confidence);
	...
	*weigheted_yi = yi_weight / total_weight;
	...
	//microsacada
	g_xi_micro_weight = g_xi_micro_weight / total_weight_conf_thresholded;
	g_yi_micro_weight = g_yi_micro_weight / total_weight_conf_thresholded;

	return (total_weight);
}

double
compute_weigheted_max_neighbor(double *weigheted_xi, double *weigheted_yi, NEURON *n, int w, int h, double log_factor)
{
	int u, v;
	double u_max = w/2, v_max = h/2, xi, yi, xi_max = 0.0, yi_max = 0.0;
	...
	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			value = nl_activation_map_hough_v1.neuron_vector[v * w + u].output.fval;
		//if (value > 0) printf("value %f\n", value);
			if (value > value_max){
				map_v1_to_image_d(&xi, &yi, IMAGE_WIDTH_RESIZED, IMAGE_HEIGHT_RESIZED, u, v, w, h, 0, 0, 0, log_factor);
				value_max = value;
				u_max = u;
				v_max = v;
				xi_max = xi;
...
//printf("xim %lf yim %lf xim %lf yim %lf u %lf v %lf \n", xi, yi, xi_max, yi_max, u_max, v_max);
	g_confidence = value_max;
	
	*weigheted_xi = xi_max/count;
	*weigheted_yi = yi_max/count;
...
output_handler_weighted_average_value_position(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	compute_weigheted_max_neighbor(&xi, &yi, output->neuron_layer->neuron_vector, w, h, log_factor);
...
init_nl_landmark_eval_mask_for_table(NEURON_LAYER *nl_landmark_eval_mask)
{
	...
	for (y = 0; y < h; y++)
	{
		for (x = 0; x < w; x++)
		{
			...
			else if (((double) x >= leftCol_gt - 0.3*ground_truth_w) &&
			    ((double) x <= rightCol_gt + 0.3*ground_truth_w) &&
			    ((double) y >= bottomRow_gt - 0.3*ground_truth_h) &&
			    ((double) y <= topRow_gt + 0.3*ground_truth_h))
				neuron_vector[y * w + x].output.ival = PIXEL (255, 0, 0);
			else
...
GetXOctante(int i)
{
	return (int)(IMAGE_WIDTH_RESIZED / 8 + ((i % 4) * IMAGE_WIDTH_RESIZED / 4));
...
{
	if (i <= 3)
		return (int)(IMAGE_HEIGHT_RESIZED / 4);
	else
		return (int)(IMAGE_HEIGHT_RESIZED / 4 + IMAGE_HEIGHT_RESIZED / 2);		
...
f_keyboard(char *key_value)
{
	char key;
	int x, y, v, u, i;
	int w_out = in_pattern_filtered_translated.dimentions.x;
	int h_out = in_pattern_filtered_translated.dimentions.y;
	int w = nl_movement_map.dimentions.x;
	int h = nl_movement_map.dimentions.y;
	int r, g, b;
...
/*			for (v = 0; v < h; v++)
			{
				...
				{
					map_v1_to_image(&x, &y, w_out, h_out, u, v, w, h, (double) w_out / 2.0, (double) h_out / 2.0, (double) h / (double) (h - 1), LOG_FACTOR);
					r = 0.5 + 255.0 * x/(double)IMAGE_WIDTH_RESIZED;
					g = 0.5 + 255.0 * y/(double)IMAGE_HEIGHT_RESIZED;
					nl_movement_table.neuron_vector[i].output.ival = PIXEL(r, g, 0);
					i++;
				}
			}
*/			
			for (v = 0; v < h; v++)
			{
				for (u = 0; u < w; u++)
				{
					b = BLUE(table_v1.neuron_vector[i].output.ival);
					nl_movement_table.neuron_vector[i].output.ival = PIXEL(0, 0, (b>0?255:0));
					i++;
				}
			}
			break;
		case 'v':
			for (i = 0; i < nl_movement_map.num_neurons; i++)
			{
				train_neuron(&nl_movement_map, i);
			}
			break;
		case 'C':	
		case 'c':
		...
		case 's':
			...
			g_image_info[g_nCurrentImageInfo].best_confidence_blue = g_confidence_blue;
			if (g_confidence < CONFIDENCE_LEVEL){
				in_pattern.wxd = x;
				in_pattern.wyd = y;
				set_input_layer_translation(&in_pattern, in_pattern.wxd, in_pattern.wyd);
