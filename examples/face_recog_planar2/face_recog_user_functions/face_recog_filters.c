...
void
open_cv_neuron_layer_resize(NEURON_LAYER *nl_output, NEURON_LAYER *nl_input)
{
	...
	cvResize(image_input, image_output, CV_INTER_NN);
...
void
neuron_layer_resize(NEURON_LAYER *nl_output, NEURON_LAYER *nl_input)
{
	int wi, hi, wo, ho, xi, yi, xo, yo;
	...
	for (yo = 0; yo < ho; yo++)
	{
		for (xo = 0; xo < wo; xo++)
		{
			xi = (int) (((double) xo / (double) wo) * (double) wi);
			yi = (int) (((double) yo / (double) ho) * (double) hi);
...
void face_reshape_filter2 (FILTER_DESC *filter_desc)
{
	PARAM_LIST *p_list = NULL;
	NEURON_LAYER_LIST *n_list = NULL;
	NEURON_LAYER *nl_output = NULL, *nl_input = NULL, *nl_input2 = NULL;
	KERNEL *gaussian_kernel = NULL;
	int nl_number, p_number;
	float baseline_factor, target_baseline, x_offset, y_offset;
	float face_centre_x, face_centre_y, scale_factor, alpha;
...
/*	if (p_number != 4)
	...
	// Gets the Input Neuron Layer
	nl_input = filter_desc->neuron_layer_list->neuron_layer;
	nl_input2 = filter_desc->neuron_layer_list->next->neuron_layer;
...
*/		}
	...
	// Reduce to 28x28 and back
	open_cv_neuron_layer_resize(nl_input2, nl_output);
	open_cv_neuron_layer_resize(nl_output, nl_input2);
	// neuron_layer_resize(nl_input2, nl_output);
	// neuron_layer_resize(nl_output, nl_input2);
}

void face_reshape_opencv (FILTER_DESC *filter_desc)
{
