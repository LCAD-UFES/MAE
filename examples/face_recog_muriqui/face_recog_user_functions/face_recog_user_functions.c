...
char *g_chPersonID[] =
{
	"m-000", "m-001", "m-002", "m-003", "m-004", "m-005", "m-006", "m-007", "m-008", "m-009", "m-010", 
};
...
int GetNextFileName(char *strFileName, int nDirection)
{
	...
	{
		printf("Error: Could not read face %s\n", strFileName);
		return (1);
	}
}
...
int ReadFaceInput_old(INPUT_DESC *input, char *strFileName)
{
	int i, nInd;
	int x, y;
	FILE *pFile = NULL;
...
void init_face_recog(INPUT_DESC *input)
{
	...
	if (!GetNextFileName(strFileName, DIRECTION_NONE))
	{
...
NEURON_OUTPUT GetFace2 (PARAM_LIST *pParamList)
{
	...
	sprintf(strFileName, "%s%s%s.pnm", INPUT_PATH, g_chPersonID[g_nPersonID], g_chIllumination[g_nIllumination]);
