...
void
print_sample_values_and_returns(int input_sample, int now)
{
	int sample;
	
	printf ("now %04d(%s) =   %3.2f:  ", now, g_data_set[now].minute, g_data_set[now].sample_price);
	for (sample = now - (NETWORK_INPUT_SIZE - 1) * NETWORK_INPUT_STRIDE; sample <= now; sample += NETWORK_INPUT_STRIDE)
	{
		if (sample >= 0)
			printf ("%04d(%s)   %3.2f   ", sample, g_data_set[sample].minute, g_data_set[sample].sample_price);
		else
			printf ("%04d(%s)            ", sample, g_data_set[0].minute);
	}
	printf ("\n");

	printf ("now %04d(%s) = % .5f:  ", now, g_data_set[now].minute, g_data_set[now].sample_return);
	for (sample = now - (NETWORK_INPUT_SIZE - 1) * NETWORK_INPUT_STRIDE; sample <= now; sample += NETWORK_INPUT_STRIDE)
	{
		if (sample >= 0)
			printf ("%04d(%s) % .5f   ", sample, g_data_set[sample].minute, g_data_set[sample].sample_return);
		else
			printf ("%04d(%s)            ", sample, g_data_set[0].minute);
	}
	printf ("\n");
...
ReadSampleInput(INPUT_DESC *input, int input_sample)
{
...
#ifdef	VERBOSE2
	print_sample_values_and_returns(input_sample, now);
#endif
	sample = now - (NETWORK_INPUT_SIZE - 1) * NETWORK_INPUT_STRIDE;
#ifdef	VERBOSE
	printf("\n");
	...
	for (x = 0; x < x_w; x++)
	{
		if ((((x - stride/2) % stride) == 0) && (sample <= now))
		{
			...
			sample_return = compute_sample_return_new (now, sample);
		    	normalized_sample_return = (sample_return - MIN_RETURN) / (MAX_RETURN - MIN_RETURN);
 			for (y = 0; y < y_h; y++)
			{
