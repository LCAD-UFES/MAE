...
print_test_samples ()
{
	...
	for (i = SAMPLES2CALIBRATE; i < N; i++)
	{
		...
		if ((g_global_data_set[0][i].wnn_predicted_return > 0.0) && // nao faz negocio de predicao zero ou negativa
		    (g_global_data_set[0][i].target_return != 0.0))
		{
...
show_statistics ()
{
	...
	for (i = SAMPLES2CALIBRATE; i < N; i++)
	{
		...
		tgt = g_global_data_set[0][i].target_return;
		out = g_global_data_set[0][i].wnn_predicted_return;
...
sumariza_desempenhos (int semana, float *desempenho)
{
	...
	for (a = 1; a < g_argc; a++)
	{
		...
		for (i = semana-SAMPLES2CALIBRATE; i < ultima_semana; i++)
		{
			tgt = g_global_data_set[a][i].target_return;
			out = g_global_data_set[a][i].wnn_predicted_return;
...
//		media_desempenhos[a] = (desempenho[a] + 4.0 * desempenho_corrente[a]);
//		media_desempenhos[a] = (desempenho_corrente[a] / (1.0 - desempenho[a]));
