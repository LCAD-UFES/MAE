...
get_next_input()
{
	...
	if (g_nStatus == TRAINING_PHASE)
		...
		Erro("Invalid net status in get_next_input()", "", "");
...
get_file_lines_len (char *file)
{
	...
	result = fscanf(linhas, "%d %*s\n", &len);
	result = system("rm linhas.deleteme");
	if (result < 0) return 0;
...
output_handler_draw_receptive_filters_center(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	for (v = 0; v < h; v = v + 3)
	{
		for (u = 0; u < w; u = u + 3)
		{
			...
			output->rectangle_list[counter].w = 0.5;
			output->rectangle_list[counter].h = 0.5;
...
NEURON_OUTPUT
CalcStatistics(PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;
#if VERBOSE
	float hit_percent;
...
GetSignByIndex (PARAM_LIST *pParamList)
{
	...
	if (g_nStatus == TRAINING_PHASE)
		g_nCurrentImageDatasetTrain = line_number;
	else
		g_nCurrentImageDatasetTest = line_number;
...
NEURON_OUTPUT
GetNextSign (PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;
...
NEURON_OUTPUT
SetNeuronMemorySize (PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;
	int	new_neuron_memory_size;

	new_neuron_memory_size = pParamList->next->param.ival;

	NEURON_MEMORY_SIZE = new_neuron_memory_size;
...
ComputeAverageTimeForNeuronsFindingNearestPattern (PARAM_LIST *pParamList)
{
	...
	for (i = 0; i < nl_landmark.num_neurons; i++)
		output.fval += nl_landmark.neuron_vector[i].time_metrics;

	output.fval /= (float)nl_landmark.num_neurons;
...
ComputeAverageNeuronLayerHashCollisionsInTrain (PARAM_LIST *pParamList)
{
	...
	output.fval = (float) count_neuron_layer_average_hash_collisions (nl_landmark.name);
...
ComputeAverageNeuronLayerHashCollisionsInTest (PARAM_LIST *pParamList)
{
	...
	for (i = 0; i < nl_landmark.num_neurons; i++)
		output.fval += nl_landmark.neuron_vector[i].hash_collisions;
