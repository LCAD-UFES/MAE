		...
		for (x = 0; x < w; x++)
		{
			...
			switch (image->nChannels)
			{
				case (3):
					...
					neuron_vector[(xo+x) + (yo+y)*h].output = neuron_output;
					break;
				case (1):
					neuron_output.ival = (int) image_pixel[3*x];
					neuron_vector[(xo+x) + (yo+y)*h].output = neuron_output;
					break;
				default:
...
copy_neuron_layer_to_image(IplImage *image, CvRect *neuron_layer_roi, NEURON_LAYER *neuron_layer)
{
	...
	for (y = h-1; y >= 0; y--)
	{
		for (x = 0; x < w; x++)
		{
			neuron_output = neuron_vector[(xo+x) + (yo+y)*neuron_layer->dimentions.y].output;
			image_pixel = (uchar*) (image->imageData + (h-1-y) * image->widthStep);
			...
			case COLOR:
				...
				break;
			case GREYSCALE:
				image_pixel[3*x] = neuron_output.ival;
				break;
			default:
