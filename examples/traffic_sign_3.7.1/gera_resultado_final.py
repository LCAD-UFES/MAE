...
arq_output = open("resultadoFinal.csv", 'w')

N_RECALL_TRIES = 1

# Lendo os arquivos
r = open("traffic_sign_RED/hit_statistics.csv")
g = open("traffic_sign_GREEN/hit_statistics.csv")
b = open("traffic_sign_BLUE/hit_statistics.csv")
...
arquivo = []
for i in range (0, len(arquivo_red)/(43*N_RECALL_TRIES)):
	j = i*43*N_RECALL_TRIES
...
for i in range(0, (len(classe_red)/(43*N_RECALL_TRIES))):
	classe_red_aux = []
	classe_green_aux = []
	classe_blue_aux = []
	...
	for j in range (0, 43*N_RECALL_TRIES):
		# print "iteracao", (i*43*N_RECALL_TRIES)+j
		classeR = int(classe_red[(i*43*N_RECALL_TRIES)+j])
		classe_red_aux[classeR] = classe_red_aux[classeR] + confianca_red[(i*43*N_RECALL_TRIES)+j]
		classeG = int(classe_green[(i*43*N_RECALL_TRIES)+j])
		classe_green_aux[classeG] = classe_green_aux[classeG] + confianca_green[(i*43*N_RECALL_TRIES)+j]
		classeB = int(classe_blue[(i*43*N_RECALL_TRIES)+j])
		classe_blue_aux[classeB] = classe_blue_aux[classeB] + confianca_blue[(i*43*N_RECALL_TRIES)+j]
