...
input_controler(INPUT_DESC *input, int status)
{
	...
	if ((input->mouse_button == GLUT_RIGHT_BUTTON) &&
			...
			(draw_active == 1))
	{
		sprintf(command, "draw char_discr_out_1 based on character move;");
		interpreter(command);
		sprintf(command, "draw char_discr_out_2 based on character move;");
		interpreter(command);
		sprintf(command, "draw char_discr_out_3 based on character move;");
		interpreter(command);
	}
...
EvaluateChar(PARAM_LIST *pParamList)
{
	...
	g_number = evaluate_output(&character_label_out, g_result, &g_difference_between_highest_second_highest);
	g_certainty = (float) g_difference_between_highest_second_highest / (float) g_result[g_number];
	g_activation = (float) g_result[g_number] / ((float) ((&character_label_out)->ww * (&character_label_out)->wh));
...
NEURON_OUTPUT
LoadLabelImage(PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;
	char command[256];

	sprintf(command, "load char_discr_%d from %d.ppm; forward char_discr_out_%d;", pParamList->next->param.ival, g_label, pParamList->next->param.ival);
	interpreter(command);
