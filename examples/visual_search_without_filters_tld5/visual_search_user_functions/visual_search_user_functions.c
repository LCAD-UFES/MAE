...
#define MIN_CONFIANCE_TO_RETRAIN 0.65
...
double
compute_confidence_background(NEURON *n, int g_halph_band_width, int w, int h)
{
	double count, confidence;
	int u, v, value;
	float altura, largura;
...
double
compute_confidence_background_percent(NEURON *n, int g_halph_band_width, int w, int h)
{
	double count, confidence;
	int u, v, value;
	float altura, largura;
	...
	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			if ( fabs(w/2.0 - u) < dist2Center[v] + 0.5 * w ){
				value = BLUE(n[v * w + u].output.ival);
				if ( BLUE(table_v1.neuron_vector[v * w + u].output.ival) > 0 ){
					if (value > 0)
					{
...
*/
	//OUTRAS MEDIDAS DE CONFIANCA
	//g_confidence_blue = compute_confidence_blue(nl_v1_activation_map.neuron_vector, g_halph_band_width, w, h);
	//g_confidence = compute_confidence_blue(nl_v1_activation_map.neuron_vector, g_halph_band_width, w, h);
	g_confidence = compute_confidence_color(nl_v1_activation_map.neuron_vector, g_halph_band_width, w, h);
	//g_confidence = compute_confidence_background(nl_v1_activation_map.neuron_vector, g_halph_band_width, w, h);
	//g_confidence = compute_confidence_background_percent(nl_v1_activation_map.neuron_vector, g_halph_band_width, w, h);
...
init_nl_landmark_eval_mask_for_table(NEURON_LAYER *nl_landmark_eval_mask)
{
	...
	leftCol_gt = ((double)(w/2.0)) - ((double)((ground_truth_w / 2.0)));
	rightCol_gt = ((double)(w/2.0)) + ((double)((ground_truth_w / 2.0)));
	bottomRow_gt = ((double)(h/2.0)) - ((double)((ground_truth_h / 2.0)));
	topRow_gt = ((double)(h/2.0)) + ((double)((ground_truth_h / 2.0)));
	...
	for (y = 0; y < h; y++)
	{
		for (x = 0; x < w; x++)
		{
			    ...
			    (y < topRow_gt))
			{
				...
				else b = ((raio - distancia) / (raio - raio_corrigido)) * 255.0;
				
			}
			else
				b = 0.0;

			g = (int)(((double)x) * (255.0 / w) + 0.5);
			r = (int)(((double)y) * (255.0 / h) + 0.5);
...
void
init_nl_landmark_eval_mask_for_color(NEURON_LAYER *nl_landmark_eval_mask)
{
	int w, h;
	double ground_truth_w;
	double ground_truth_h;
...
//	init_nl_landmark_eval_mask_for_color(&table);
