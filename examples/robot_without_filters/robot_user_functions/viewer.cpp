...
GLint g_nMapWidth = 1280; // [pix]
GLint g_nMapHeight = 240; // [pix]
...
GLfloat g_fltFloor = 10.0f; // [mm]
GLfloat g_fltViewerRobotRadius = 15.0f; // [mm]
GLfloat g_fltViewerTerrainSize = 250.0f; // [mm]
	...
	switch (key)
	{
		...
		case '+':
			...
			break;
                ...
                case 'L':
                case 'l':
                        if (!g_pMap->Load ((char *) MAP_FILE_NAME))
                ...
                case 'D':
                case 'd':
                        if (!g_pMap->Save ((char *) MAP_FILE_NAME))
                ...
                case 'S':
                case 's':
                        if (!g_pMap->SaveImage ((char *) IMAGE_FILE_NAME))
