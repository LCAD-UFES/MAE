#include <locale.h>
#include "neural_global_localizer_streetview_user_functions.h"
...
Converte imagem 2 -> 001_1 etc...
(1 a 8 imagens de angulações diferentes 0 a 360 graus definido pelo parametro n_images).

*/
char* g_frame_ID_streetview(int g_frameID, int n_images)
{
	char new_filename[100];
	int rotation_num = g_frameID%n_images - 1;
	int ID_streetview = g_frameID/n_images + 1; 

	if(rotation_num == -1)
	{
		rotation_num = n_images-1;
		ID_streetview--;
	}

	sprintf (new_filename, "%03d_%d", ID_streetview, rotation_num);
	return new_filename;
}

/*
Recebe um ID no formato streetview e converte para o global.
ID 250_7 -> 2000
ID 194_5 -> 1550
*/
int g_streetview_to_frame_ID(char *g_frameID_streetview)
{
    int frameID = 0,
        rotation_num = 0,
        ID_streetview = 0,
    ...
    char str[15];
    strcpy(str, g_frameID_streetview);
    ...
    while (pch != NULL)
      {
        if(cont == 0)
        {
            ID_streetview = atoi(pch);
        }
        else
        {
            rotation_num = atoi(pch);
        }

        pch = strtok (NULL, " ,.-_");
        cont++;
    }
    //printf ("%d .... %d\n",ID_streetview, rotation_num);

    if(rotation_num == 7)
    {
        frameID = ID_streetview*8;
    }
    else if(rotation_num == 6)
    {
        frameID = ID_streetview*8 - 1;
    }
    else if(rotation_num == 5)
    {
        frameID = ID_streetview*8 - 2;
    }
    else if(rotation_num == 4)
    {
        frameID = ID_streetview*8 - 3;
    }
    else if(rotation_num == 3)
    {
        frameID = ID_streetview*8 - 4;
    }
    else if(rotation_num == 2)
    {
        frameID = ID_streetview*8 - 5;
    }
    else if(rotation_num == 1)
    {
        frameID = ID_streetview*8 - 6;
    }
    else if(rotation_num == 0)
    {
        frameID = ID_streetview*8 - 7;
    }
...
int get_next_file_name(char *strFileName, char* directory, int nDirection, int number_of_frames)
{
	...
	while (pFile == NULL)
	{
		//char *streetview_ID = g_frame_ID_streetview(g_frameID, 8);//usar com streetview360
		//sprintf (strFileName, "%s%s.ppm", directory, streetview_ID);
	...
	if (g_networkStatus == RECALL_PHASE)
	{
		if (strcmp (output->name, out_nl_v1_pattern.name) == 0)
		{
			...
			if(g_has_new_frame == 1)
			{
				...
				//char *streetview_frame_id = malloc(sizeof(char) * 100);//usar com streetview360
				//streetview_frame_id = g_frame_ID_streetview(winner_frame, 8);
				//sprintf(recall_filename, "%s%s.ppm", INPUT_TRAINING_PATH, streetview_frame_id);

				sprintf(recall_filename, "%s%03d.ppm", INPUT_TRAINING_PATH, winner_frame);
