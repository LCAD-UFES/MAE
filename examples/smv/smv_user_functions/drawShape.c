...
+-----------------------------------------------------------------------
|
| selectionPointsInitialize - Iniatilize the vector needed to memorize all points select by the use
| Input:	maxSize - max points that can be selected by the user.
| Output:	void
|
+-----------------------------------------------------------------------
*/
...
/* void
selectionPointsInitialize (int maxSize)
{
	selectedPoint.maxSize = maxSize;
	selectedPoint.lastDominantEyePoint = 0;
	selectedPoint.generateShape = POINTS_NOT_SELECTED;
	selectedPoint.dominantEyePoints = NULL;
	selectedPoint.nonDominantEyePoints = NULL;
}
 */
...
+-----------------------------------------------------------------------
|
| setNewSizeOfSelectionPoints - change the size of selectionPoints array.
| Input:	newSize - the new size of the selectionPoints
| Output:	void
|
+-----------------------------------------------------------------------
*/
...
/* void
setNewSizeOfSelectionPoints (int newSize)
{
	int newLastDominantEyePoint = 0;
	XY_PAIR *newSelectionPoints = NULL;
	int i;
	
	newSelectionPoints = (XY_PAIR *) alloc_mem ((size_t) sizeof (XY_PAIR) * newLastDominantEyePoint);
	
	for (i = 0;i < selectedPoint.lastDominantEyePoint; i++)
	{
		newSelectionPoints.x = 
	}
}
 */
...
/*
-----------------------------------------------------------------------------------------------------
comparePoints - this function will compare two points of shape map.
Input	- const void * x - inh-element of shape map.
	  const void * y - i+1nh-element of shape map.
Output	- int - if x > y return positive number else if x =  y return 0 else return negative number
-----------------------------------------------------------------------------------------------------
*/
/* 
int
comparePoints (const void *x, const void *y)
{
	const MAP_POINT *first_element = x, *second_element = y;
	double firstX, secondX;
	firstX	= (*first_element).point.x;
	secondX	= (*second_element).point.x;
...
/*
------------------------------------------------------------------------------------------
prepareMap - this function will prepare map reallocating the TURN_OFF points and sorting
	     the TURN_ON points by the X axis.
Input	- Map used by shape.
Output	- No outputs.
------------------------------------------------------------------------------------------
*/

/* void
prepareMap (SHAPE_MAP *shape_map)
{
	int i;
	WORLD_POINT current_point;
	
	i = shape_map->totalPoints - 1;
	while(i >= 0 && shape_map->map_point[i].status == TURN_OFF)
	{
		shape_map->totalPoints--;
		i--;
	}
	...
	i = 0;
	while(i < shape_map->totalPoints)
	{
		if (shape_map->map_point[i].status == TURN_OFF)
		{
			shape_map->map_point[i].point.x = shape_map->map_point[shape_map->totalPoints - 1].point.x;
			shape_map->map_point[i].point.y = shape_map->map_point[shape_map->totalPoints - 1].point.y;
			shape_map->map_point[i].point.z = shape_map->map_point[shape_map->totalPoints - 1].point.z;
			shape_map->map_point[shape_map->totalPoints - 1].point.x = (double) 0.0;
			shape_map->map_point[shape_map->totalPoints - 1].point.y = (double) 0.0;
			shape_map->map_point[shape_map->totalPoints - 1].point.z = (double) 0.0;
			shape_map->map_point[shape_map->totalPoints - 1].status = TURN_OFF;
			shape_map->totalPoints--;
		}
		i++;
	}
	
	qsort(shape_map->map_point, shape_map->totalPoints - 1, sizeof(MAP_POINT), comparePoints);
}
 */
/*
------------------------------------------------------------------------------------------
