...
#define RAMDOM_IMAGES_TRAIN	"/dados/tld/images/image_list.txt"
#define RAMDOM_IMAGES_RECALL	"/dados/tld/images/image_list.txt"
#define GT_CREATE_FILE          "/dados/tld/gt_created.txt"
...
read_images_info_file(char *file_name)
{
    ...
    for (g_num_image_info = 0; g_num_image_info < num_lines; g_num_image_info++)
    {
        if (fgets(file_line, 256, image_list))
        {
            ...
            {
                sscanf(file_line, "image%d.ppm;%d;%d;%d;%d;%d", &(g_image_info[g_num_image_info].ImgNo), &(g_image_info[g_num_image_info].leftCol),
            ...
            {
                sscanf(file_line, "image%d.ppm", &(g_image_info[g_num_image_info].ImgNo));
...
f_keyboard(char *key_value)
{
    ...
    case 'P':
        ...
        fprintf(gt, "image%05d.pnm,%d,%d\n", g_image_info[g_nCurrentImageInfo].ImgNo, in_pattern.wxd, in_pattern.wyd);
        fprintf(stderr, "X: %d\n", in_pattern.wxd);
        fprintf(stderr, "Y: %d\n", in_pattern.wyd);
        fclose(gt);
        break;
