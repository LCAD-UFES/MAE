...
int EvaluateOutputGaussianRGB(OUTPUT_DESC *outputR, OUTPUT_DESC *outputG, OUTPUT_DESC *outputB, float *confidence)
{
	int i;
	int nPersonID = 0;
	int nMax = 0;
	...
	for (i = 0; i < (outputR->wh * outputR->ww); i++)
	{
		if ((neuron_vector[i].output.ival >= 0) && (neuron_vector[i].output.ival < (MAX_MAN_ID+MAX_WOMAN_ID)))
			g_vCountPerson[neuron_vector[i].output.ival] += 1;
	}
	...
	for (i = 0; i < (outputG->wh * outputG->ww); i++)
	{
		if ((neuron_vector[i].output.ival >= 0) && (neuron_vector[i].output.ival < (MAX_MAN_ID+MAX_WOMAN_ID)))
			g_vCountPerson[neuron_vector[i].output.ival] += 1;
	}
	...
	for (i = 0; i < (outputB->wh * outputB->ww); i++)
	{
		if ((neuron_vector[i].output.ival >= 0) && (neuron_vector[i].output.ival < (MAX_MAN_ID+MAX_WOMAN_ID)))
			g_vCountPerson[neuron_vector[i].output.ival] += 1;
	}
	...
	*confidence = (float) nMax / (float) ((outputR->wh * outputR->ww) + (outputG->wh * outputG->ww) + (outputB->wh * outputB->ww));
	return (nPersonID);
}
...
void output_handler_old (OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	{
		// Avalia a saida
		if (strcmp (output->name, out_landmark_blue.name) == 0)
		{
			g_nTotalTested++;
	...
	if (g_nStatus == RECALL_PHASE)
	{
		...
		if (strcmp (output->name, out_landmark_blue.name) == 0)
		{
			if (g_nTry != 0)
			{
				...
				person = EvaluateOutputGaussianRGB(&out_landmark_red,&out_landmark_green,&out_landmark_blue,&confidence);
