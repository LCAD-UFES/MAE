...
#define	SIGN_COUNTER_MAX 20

int GetNextTrafficSignFileName(char *strFileName, int nDirection)
{
	FILE *pFile = NULL;
	static int Sign_cnt = 0;	//Null number
	...
	if(nDirection == DIRECTION_FORWARD)
	{
		if(Sign_cnt == SIGN_COUNTER_MAX)
			Sign_cnt = 1;	//first valid photo number
		else
			Sign_cnt++;
	}
	else
	{
		if(Sign_cnt == 1)
			Sign_cnt = SIGN_COUNTER_MAX;
		else
			Sign_cnt--;
	}

	sprintf (strFileName, "%s%d.pnm", INPUT_PATH,Sign_cnt);
	printf ("FileName: %s\n", strFileName);
	fflush (stdout);
...
int ReadFaceInput_other(INPUT_DESC *input, char *strFileName)
{
	int i, nInd;
	int x, y;
	FILE *pFile = NULL;
...
int GetNewFace(INPUT_DESC *input, int nDirection)
{
	...
	while (GetNextTrafficSignFileName(strFileName, nDirection))
		;
...
/*
***********************************************************************************
* Function: output_handler_centroid_weighted_inverse_log_polar_position           *
* Description:                                         				  *
* Inputs:                                              				  *
* Output:                                              				  *
***********************************************************************************
...
void output_handler_centroid_weighted_inverse_log_polar_position (OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	char *nl_target_coordinates_name = NULL;
	...
	for (v = 0 ; v < h; v++)
		...
		{
			map_v1_to_image (&xi, &yi, IMAGE_WIDTH, IMAGE_HEIGHT, u, v, w , h, 0, 0, (double) h / (double) (h - 1), log_factor);
			//map_v1_to_image (&xi, &yi, IMAGE_WIDTH, IMAGE_HEIGHT, u, v, w , h, IMAGE_WIDTH/2 , IMAGE_HEIGHT/2 , (double) h / (double) (h - 1), log_factor);
			mass_accumulator += output->neuron_layer->neuron_vector[v * w + u].output.fval;
			u_accumulator += xi*(output->neuron_layer->neuron_vector[v * w + u].output.fval);
			v_accumulator += yi*(output->neuron_layer->neuron_vector[v * w + u].output.fval);
		}
	}
	...
	nl_target_coordinates->neuron_vector[0].output.fval = (float)((int) (u_centroid));
	nl_target_coordinates->neuron_vector[1].output.fval = (float)((int) (v_centroid));
}
