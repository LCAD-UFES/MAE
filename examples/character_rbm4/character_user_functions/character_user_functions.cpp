...
std::vector<Vector> g_data_set_train_x(60000);
std::vector<Vector> g_data_set_train_y(60000);
...
void
update_stats(int number, int label, float certainty, float result[10])
{
	int i;
	...
	for (i = 0; i < 10; i++)
	{
		printf("%d = %0.2f  ", i, result[i]);
	}

	if (number == label)
	{
		correct++;
		printf("-> %d == %d  %0.2f (%0.4f)\n",
				number, label,
				certainty,
...
rbm_filter(FILTER_DESC *filter_desc)
{
	...
	{
		network = new DeepBeliefNet();
		p_network = (DeepBeliefNet **) &(filter_desc->filter_params->next->param.pval);
		*p_network = network;

		network->build(std::vector<int>{data_size, 500, 500, 2000, 10});

		auto& rbm = network->output_layer();
	...
	{
		network = (DeepBeliefNet *) filter_desc->filter_params->next->param.pval;
	}

	if (g_state == READ_ALL)
	{
		Vector& input = g_data_set_train_x[g_cont_char];
		Vector& target = g_data_set_train_y[g_cont_char];
	...
	{
		LRBM::Conf conf;
		conf.max_epoch_ = 50; conf.max_batches_ = 600; conf.batch_size_ = 100;
		network->pretrain(g_data_set_train_x, conf, 0);
		network->backprop(g_data_set_train_x, g_data_set_train_y, conf, 0);
	}
	else if (g_state == TEST)
	{
		...
		network->predict(input, nil, output);
...
int
evaluate_output(OUTPUT_DESC *output, float *result)
{
	NEURON *neuron;
	...
	{
		result[i] = neuron[i].output.fval;
		if (neuron[i].output.fval > greater)
		{
			greater = neuron[i].output.fval;
			number = i;
		}
	}
...
EvaluateChar(PARAM_LIST *pParamList)
{
	...
	g_number = evaluate_output(&character_RBM_out, g_result);
	g_certainty = g_result[g_number];
...
UpdateStats(PARAM_LIST *pParamList)
{
	...
	update_stats(g_number, g_label, g_certainty, g_result);
