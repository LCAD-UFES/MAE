...
view_3D_cylinder (CYLINDER *cylinder, int arcs, int circles)
{
	...
	double *world3DCylinderLeft = (double*) alloc_mem (3 * num_points * sizeof (double));

	compute_3D_cylinder (cylinder, world3DCylinderLeft, arcs, circles, 360);
	for (i = 0; i < num_points; i++)
	{
		// Updates the map and rotates the frame coordinates			       			
		ViewerUpdateMap(1, world3DCylinderLeft[3*i+2],
		                  -world3DCylinderLeft[3*i+0],
				  world3DCylinderLeft[3*i+1], 0, 0, 0);
	}
...
{
	double centers_distance = sqrt(POW2(c1->x - c2->x) +
				       POW2(c1->y - c2->y) +
				       POW2(c1->z - c2->z));
