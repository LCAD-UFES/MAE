...
void ReconstructByDisparityMapPlan_new()
{
...
//	display_points (num_points / 2, p_dblLeftPoint, p_dblRightPoint);
	...
	for (xo = 0; xo < w; xo++)
	{
		...
		for (yo = 0; yo < h; yo++)
		{
			...
			ViewerUpdateMap(0, p_dblLeftWorldPoint[num_points+2], -p_dblLeftWorldPoint[num_points+0], p_dblLeftWorldPoint[num_points+1], cRed, cGreen, cBlue);
			num_points += 3;
		}
	}
	free (p_dblLeftPoint);
...
void ReconstructByDisparityMapPlan_old_Stiven()
{
	int w, h, wi, hi, xi, yi, yo, xo, num_points;
	float fltDisparity;
	int pixel;
...
//			yi = (int) ((float) (yo * hi) / (float) h + 0.5);
			...
			// Updates the map and rotates the frame coordinates			       			
			ViewerUpdateMap(0, p_dblRightWorldPoint[2], -p_dblRightWorldPoint[0], -p_dblRightWorldPoint[1], cRed, cGreen, cBlue);
		}
	}
