...
void
get_input_by_file_line_number_gtsrb(int line_number)
{
...
void
get_input_by_file_line_number_gtsdb(int line_number)
{
	int roi_x1, roi_y1, roi_x2, roi_y2, i;
	int TrafficSignId, TrafficSignType, IsTrafficSign;
	char file_name[256];
	char file_path[256];
	...
	while (i < line_number)
	{
		if (fscanf (g_signs_data_file, "%s\n",file_name))
			;
		if (sscanf (file_name, "%d_%d_%d_%d_%d_%d_%d_%d\n", &g_signID, &roi_x1, &roi_y1, &roi_x2, &roi_y2, &TrafficSignId, &TrafficSignType, &IsTrafficSign) != 8)
			Erro("Could not read line from signs_data_file in get_sign_by_index()", "", "");
		i++;
	}

	g_signUniqueID = TrafficSignType;
	g_img_w = abs(roi_x1-roi_x2);
	g_img_h = abs(roi_y1-roi_y2);
	g_roi_x1 = 0;
	g_roi_y1 = 0;
	g_roi_x2 = g_img_w;
	g_roi_y2 = g_img_h;
...
{
	if (g_nStatus == TRAINING_PHASE)
		get_input_by_file_line_number_gtsrb(line_number);
	else if (g_nStatus == RECALL_PHASE)
		get_input_by_file_line_number_gtsrb(line_number);
	else
		Erro("Invalid net status in get_input_by_file_line_number()", "", "");
...
void input_controler (INPUT_DESC *input, int status)
{
	...
	if ((input->mouse_button == GLUT_RIGHT_BUTTON) &&
	    ...
	    (draw_active == 1))
	{
		sprintf (strCommand, "draw out_landmark_red based on out_traffic_gaussian move;");
		interpreter (strCommand);
		sprintf (strCommand, "draw out_landmark_green based on out_traffic_gaussian move;");
		interpreter (strCommand);
		sprintf (strCommand, "draw out_landmark_blue based on out_traffic_gaussian move;");
		interpreter (strCommand);
	}
...
EvaluateOutputForLogPolar(OUTPUT_DESC *output)
{
	...
	for (i = 0; i < output->wh; i++)
	{
		for (j = 0; j < output->ww; j++)
		{
			...
			{
				sign_count[neuron_vector[i*output->wh + j].output.ival].sign_count += 1.0 * //weight *
	...
	for (i = 0; i < N_DEFINED_SIGNS; i++)
		sign_count[i].confidence = sign_count[i].sign_count / (float) (output->wh * output->ww);

	qsort((void *) sign_count, N_DEFINED_SIGNS, sizeof(SIGN_COUNT), compare_outputs);
}

void 
EvaluateOutputForLogPolarRGB(OUTPUT_DESC *outputR,OUTPUT_DESC *outputG, OUTPUT_DESC *outputB)
{
	int i, j;
	//float weight, acc_weight;
	NEURON *neuron_vector;
	...
	neuron_vector = outputR->neuron_layer->neuron_vector;

	for (i = 0; i < outputR->wh; i++)
	{
		for (j = 0; j < outputR->ww; j++)
		{
			if ((neuron_vector[i*outputR->wh + j].output.ival >= 0) && (neuron_vector[i*outputR->wh + j].output.ival < N_DEFINED_SIGNS))
			{
				sign_count[neuron_vector[i*outputR->wh + j].output.ival].sign_count += 1.0 * normal_dist((float)j, (float)(outputR->wh/2.0), VOTE_VARIANCE);
				//weight = 1.0 * normal_dist((float)j, (float)(outputR->wh/2.0), VOTE_VARIANCE);
				//sign_count[neuron_vector[i*outputR->wh + j].output.ival].sign_count += weight;
				//acc_weight += weight;
			}
		}
	}
	
	neuron_vector = outputG->neuron_layer->neuron_vector;

	for (i = 0; i < outputG->wh; i++)
	{
		for (j = 0; j < outputG->ww; j++)
		{
			if ((neuron_vector[i*outputG->wh + j].output.ival >= 0) && (neuron_vector[i*outputG->wh + j].output.ival < N_DEFINED_SIGNS))
			{
				sign_count[neuron_vector[i*outputG->wh + j].output.ival].sign_count += 1.0 * normal_dist((float)j, (float)(outputG->wh/2.0), VOTE_VARIANCE);
				//weight = 1.0 * normal_dist((float)j, (float)(outputG->wh/2.0), VOTE_VARIANCE);
				//sign_count[neuron_vector[i*outputG->wh + j].output.ival].sign_count += weight;
				//acc_weight += weight;
			}
		}
	}
	
	neuron_vector = outputB->neuron_layer->neuron_vector;

	for (i = 0; i < outputB->wh; i++)
	{
		for (j = 0; j < outputB->ww; j++)
		{
			if ((neuron_vector[i*outputB->wh + j].output.ival >= 0) && (neuron_vector[i*outputB->wh + j].output.ival < N_DEFINED_SIGNS))
			{
				sign_count[neuron_vector[i*outputB->wh + j].output.ival].sign_count += 1.0 * normal_dist((float)j, (float)(outputB->wh/2.0), VOTE_VARIANCE);
				//weight = 1.0 * normal_dist((float)j, (float)(outputB->wh/2.0), VOTE_VARIANCE);
				//sign_count[neuron_vector[i*outputB->wh + j].output.ival].sign_count += weight;
				//acc_weight += weight;
			}
		}
	}
	...
	for (i = 0; i < N_DEFINED_SIGNS; i++)
		sign_count[i].confidence = sign_count[i].sign_count / (float) (3 * outputB->wh * outputB->ww);
		//sign_count[i].confidence = sign_count[i].sign_count / (float) (acc_weight);
...
SIGN_COUNT sign_count_candidates[NUM_CANDIDATES * N_RECALL_TRIES];
...
void output_handler(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	{
		//avalia a saida
		if (strcmp (output->name, out_landmark_red.name) == 0);
		{
			if (g_nTry != 0)
			{
				EvaluateOutputForLogPolar(output);
				for (i = 0; i < NUM_CANDIDATES; i++)
					sign_count_candidates[i + (g_nTry - 1) * NUM_CANDIDATES] = sign_count[i];

				g_nTry--;
			}
			if (g_nTry == 0)
			{
				qsort((void *) sign_count_candidates, NUM_CANDIDATES * N_RECALL_TRIES, sizeof(SIGN_COUNT), compare_outputs);
...
//* Description:
//***********************************************************
void output_handler_rgb(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	int i;
	
	if (g_nStatus == RECALL_PHASE)
	{
		//avalia a saida (ligada na camada de verde)
		if (strcmp (output->name, out_landmark_green.name) == 0);
		{
			if (g_nTry != 0)
			{
				EvaluateOutputForLogPolarRGB(output,&out_landmark_green,&out_landmark_blue);
				for (i = 0; i < NUM_CANDIDATES; i++)
					sign_count_candidates[i + (g_nTry - 1) * NUM_CANDIDATES] = sign_count[i];
...
int EvaluateOutputForLogPolar_old(OUTPUT_DESC *output, float *confidence)
{
	int i,j;
	int sign_id = 0;
	float max =0;
	...
	for (i = 0; i < output->wh; i++)
	{
		for (j = 0; j < output->ww; j++)
		{
			...
			if ((neuron_vector[i*output->wh + j].output.ival >= 0) && (neuron_vector[i*output->wh + j].output.ival < N_DEFINED_SIGNS))
			{
				sign_count[neuron_vector[i*output->wh + j].output.ival] += 1.0 * //weight *
											   normal_dist((float)j, (float)(output->wh/2.0), VOTE_VARIANCE);
			}
		}
	}
...
int EvaluateOutputForGaussian(OUTPUT_DESC *output, float *confidence)
{
	int i,j;
	int sign_id = 0;
	float max =0;
		...
		for (j = 0; j < output->ww; j++)
		{
			...
			if ((neuron_vector[i*output->wh + j].output.ival >= 0) && (neuron_vector[i*output->wh + j].output.ival < N_DEFINED_SIGNS))
			{
				sign_count[neuron_vector[i*output->wh + j].output.ival] += 1.0 * weight *
											   ...
											   normal_dist((float)j, (float)(output->ww/2.0), VOTE_VARIANCE);
			}
		}
	}
...
void output_handler_old(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	if (g_nStatus == RECALL_PHASE)
	{
		...
		//if (strcmp (output->name, out_traffic_sign.name) == 0)
		{
			if (g_nTry != 0)
			{
				sign = EvaluateOutputForLogPolar_old(output, &confidence);
				//sign = EvaluateOutputForGaussian(output, &confidence);
...
GetConfidence(PARAM_LIST *pParamList)
{
	...
	output.ival = (int) (sign_count_candidates[0].confidence * 100.0 * 100.0);
	//output.ival = (int) (sign_count_candidates[0].confidence * 100.0);
	//printf("confidence = %d\n", output.ival);
	
	if (best_sign.confidence < sign_count_candidates[0].confidence)
		best_sign = sign_count_candidates[0];
...
PrintStatistics(PARAM_LIST *pParamList)
{
	...
	if (NUM_CANDIDATES == 1)
	{
		...
		fprintf(stderr, "%05d.ppm;%d;%d;%d;%d;%d;%d;%d\n", g_signID, g_img_w, g_img_h, g_roi_x1, g_roi_y1, g_roi_x2, g_roi_y2, g_signUniqueID);//best_sign.sign_code);
		fflush (stderr);
		fflush (stdout);
	}
	else
	{
		for (i = j = 0; i < NUM_CANDIDATES; i++)
		{
			if (sign_count_candidates[j].sign_code == g_signUniqueID)
			{
				g_nCorrect++;

				hit_percent = (float) (g_nCorrect * 100.0 / g_nTotalTested);
				printf("HIT%d %4d %4d ", i, g_signUniqueID, sign_count_candidates[j].sign_code);
				printf("%4d %4d ", g_signID, g_sign_frameID);
				printf("%4d %4d %3.2f %2.5f\n", g_nTotalTested, g_nCorrect, hit_percent, sign_count_candidates[j].confidence);
				fprintf(stderr, "%05d.ppm;%d\n", g_signID, sign_count_candidates[j].sign_code);
				fflush (stderr);
				fflush (stdout);
			...
			j++;
			while (sign_count_candidates[j].sign_code == sign_count_candidates[j - 1].sign_code)
				j++;
		}
		...
		{
			hit_percent = (float) (g_nCorrect * 100.0 / g_nTotalTested);
			printf("MISS %4d %4d ", g_signUniqueID, sign_count_candidates[0].sign_code);
			printf("%4d %4d ", g_signID, g_sign_frameID);
			printf("%4d %4d %3.2f %2.5f\n", g_nTotalTested, g_nCorrect, hit_percent, sign_count_candidates[0].confidence);

			fprintf(stderr, "%05d.ppm;%d\n", g_signID, sign_count_candidates[0].sign_code);
			fflush (stderr);
			fflush (stdout);
...
SaveConfig (PARAM_LIST *pParamList)
{
	...
	if (file)
	{
		nl_output = &nl_landmark_red; //TODO: passar na lista de parâmetros
...
NEURON_OUTPUT
SaveNeuronsMemory (PARAM_LIST *pParamList)
{
	NEURON_LAYER *nl_output = NULL;
	NEURON_OUTPUT output;
	NEURON *neuron;
	...
	copy_without_trailings(filename, pParamList->next->param.sval);

	file = fopen(filename, "w");

	if (file)
	{
		nl_output = &nl_landmark_red; // so pega uma camada
		...
		for (i = 0; i < out_width * out_height; i++)
		{
			...
			if (neuron)
			{
				...
				for (j = 0; (j < NEURON_MEMORY_SIZE) && (memory[j].pattern != NULL); j++)
				{
					fwrite((void *) (memory[j].pattern), 1, N_PATTERNS, file);
				}
			}
		}
