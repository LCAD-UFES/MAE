...
void traffic_sign_reshape_opencv (FILTER_DESC *filter_desc)
{
	...
	int wi, hi, wo, ho;
	//int boarder_x, boarder_y;

	//printf ("Rotation angle = %f\n", reshape_filter_rotation_angle);

	//printf ("Scale factor = %f\n", reshape_filter_scale_factor);

	//boarder_x = (int)(0.1 * (float)g_img_w);
	//boarder_y = (int)(0.1 * (float)g_img_h);

	//image_roi = cvRect (g_roi_x1+boarder_x, g_roi_y1+boarder_y, g_roi_x2-g_roi_x1-2*boarder_x, g_roi_y2-g_roi_y1-2*boarder_y);
