...
#define SIZE_MIN_LIMIT			250
#define SIZE_MAX_LIMIT			300
...
int
is_valid_cylinder(CYLINDER *cylinder, CYLINDER_LIST *cylinder_list, double *working_area_3D)
{
	double PI = acos(-1);
	double alphaMin = PI * ((double)ALPHA_MIN_LIMIT)/180.0;
	double alphaMax = PI * ((double)ALPHA_MAX_LIMIT)/180.0;
...
estimate_volume (int *working_area, NEURON_LAYER *disparity_map, NEURON_LAYER *right_image)
{
	...
	do
	{
		...
		if (!error)
		{
			...
			if (is_valid_cylinder (&cylinder, curr_cylinder_list, working_area_3D))
			{
