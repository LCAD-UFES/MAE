...
update_simple_mono_cells_filters ()
{
	...
	for (f_list = global_filter_list; f_list != (FILTER_LIST *) NULL; f_list = f_list->next)
	{
		...
		if (strcmp(f_list->filter_descriptor->name, "sc_l1_filter") == 0)
			filter_update (f_list->filter_descriptor);

		if (strcmp(f_list->filter_descriptor->name, "sc_r1_filter") == 0)
			filter_update (f_list->filter_descriptor);

		if (strcmp(f_list->filter_descriptor->name, "sc_q_l1_filter") == 0)
			filter_update (f_list->filter_descriptor);

		if (strcmp(f_list->filter_descriptor->name, "sc_q_r1_filter") == 0)
			filter_update (f_list->filter_descriptor);


		if (strcmp(f_list->filter_descriptor->name, "sc_l2_filter") == 0)
			filter_update (f_list->filter_descriptor);

		if (strcmp(f_list->filter_descriptor->name, "sc_r2_filter") == 0)
			filter_update (f_list->filter_descriptor);

		if (strcmp(f_list->filter_descriptor->name, "sc_q_l2_filter") == 0)
			filter_update (f_list->filter_descriptor);

		if (strcmp(f_list->filter_descriptor->name, "sc_q_r2_filter") == 0)
			filter_update (f_list->filter_descriptor);
	}
}
...
{	
	...
	switch (key_value[0]) 
	{
		...
		case 'S':
		{
			...
			guess_cylinder_list (g_pWorkingArea, &nl_trunk_segmentation_map, 
					     &nl_disparity_map);
		...
		case 's':
		{
			...
			glutSetWindow(out_subpixel_disparity_map.win);
			update_output_image (&(out_subpixel_disparity_map));
			glutPostWindowRedisplay (out_subpixel_disparity_map.win);
		break;
		}
		case 'V':
		case 'v':
