...
int init_user_functions (void)
{
	...
	hit_statistics_csv_file = fopen("hit_statistics_minchinton.csv","w+");
...
void output_handler(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	if (g_nStatus == RECALL_PHASE)
	{
		...
		if (strcmp (output->name, out_landmark.name) == 0);
		{
			...
			if (g_nTry == 0)
			{
				//qsort((void *) sign_count_cadidates, NUM_CANDIDATES * N_RECALL_TRIES, sizeof(SIGN_COUNT), compare_outputs);
...
GetConfidence(PARAM_LIST *pParamList)
{
	...
	{
		j = (try - 1) * NUM_CANDIDATES;

		if (best_sign.confidence < sign_count_cadidates[j].confidence)
		{
			best_sign = sign_count_cadidates[j];
			output.ival = (int) (sign_count_cadidates[j].confidence * 100.0 * 100.0);
		}
	}
	return (output);
}
...
//			fprintf(hit_statistics_csv_file,"%05d.ppm;%4d;%2.5f\n",g_signID,best_sign.sign_code,best_sign.confidence);
		}
		else
		{
			hit_percent = (float) (g_nCorrect * 100.0 / g_nTotalTested);
...
//		{
//			if (sign_count_cadidates[j].sign_code == g_signUniqueID)
//			{
//				g_nCorrect++;
//
//				hit_percent = (float) (g_nCorrect * 100.0 / g_nTotalTested);
//				printf("HIT%d %4d %4d ", i, g_signUniqueID, sign_count_cadidates[j].sign_code);
//				printf("%4d %4d ", g_signID, g_sign_frameID);
//				printf("%4d %4d %3.2f %2.5f\n", g_nTotalTested, g_nCorrect, hit_percent, sign_count_cadidates[j].confidence);
//				fprintf(hit_statistics_csv_file,"%05d.ppm; %4d ; %2.5f\n",g_signID,best_sign.sign_code,best_sign.confidence);
//				fprintf(stderr, "%05d.ppm;%d\n", g_signID, sign_count_cadidates[j].sign_code);
//				fflush (stderr);
//				fflush (stdout);
//
//				break;
//			}
//			j++;
//			while (sign_count_cadidates[j].sign_code == sign_count_cadidates[j - 1].sign_code)
//				j++;
//		}
//
//		if (i == NUM_CANDIDATES)
//		{
//			hit_percent = (float) (g_nCorrect * 100.0 / g_nTotalTested);
//			printf("MISS %4d %4d ", g_signUniqueID, sign_count_cadidates[0].sign_code);
//			printf("%4d %4d ", g_signID, g_sign_frameID);
//			printf("%4d %4d %3.2f %2.5f\n", g_nTotalTested, g_nCorrect, hit_percent, sign_count_cadidates[0].confidence);
//			fprintf(hit_statistics_csv_file,"%05d.ppm;%4d;%2.5f\n",g_signID,best_sign.sign_code,best_sign.confidence);
//			fprintf(stderr, "%05d.ppm;%d\n", g_signID, sign_count_cadidates[0].sign_code);
//			fflush (stderr);
//			fflush (stdout);
//		}
//
