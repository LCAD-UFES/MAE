...
Train()
{
	...
	{
		vg_ram_wnn.network_input = &training_set.sample[sample * vg_ram_wnn.network_input_size];
...
Test()
{
	...
	if ((bit_pattern = (unsigned int *) malloc(vg_ram_wnn.memory_bit_group_size * PATTERN_UNIT_SIZE/8)) == NULL)
		Error("Could not allocate bit_pattern in Test().", "", "");

	if ((candidate_patterns = (int *) malloc(training_set.num_samples * sizeof(int))) == NULL)
		Error("Could not allocate candidate_patterns in Test().", "", "");

	correct = 0;
	for (sample = 0; sample < testing_set.num_samples; sample++)
	{
		vg_ram_wnn.network_input = &(testing_set.sample[sample * vg_ram_wnn.network_input_size]);
		...
		largest_function_f_value = EvaluateNetworkOutput(&network_output_class, &(vg_ram_wnn.neuron_output[sample * vg_ram_wnn.number_of_neurons]), vg_ram_wnn.number_of_neurons, sample_class);
...
main (int argc, char *argv[])
{
	...
	Save(&vg_ram_wnn, &training_set, "memory_seq.log");
