...
*/
        ...
        if (strcmp(input->name, image_left.name) == 0 && g_nShowRangeLine)
        {
            ...
            glBegin(GL_POINTS);
              for (i = 0; i <= 5; i++){
                 glVertex2f (g_pInfinityPoint[0] + i, g_pInfinityPoint[1]);
                 glVertex2f (g_pInfinityPoint[0] - i, g_pInfinityPoint[1]);
                 glVertex2f (g_pInfinityPoint[0], g_pInfinityPoint[1] - i);
                 glVertex2f (g_pInfinityPoint[0], g_pInfinityPoint[1] + i);
              }
            glEnd ();
...
// Output:
// ----------------------------------------------------------------------------
void update_not_simple_mono_cells_filters ()
{
    FILTER_LIST *f_list;

    for (f_list = global_filter_list; f_list != (FILTER_LIST *) NULL; f_list = f_list->next){
      if (strcmp(f_list->filter_descriptor->name, "nl_simple_binocular_filter") == 0){
        filter_update (f_list->filter_descriptor);
      }
      if (strcmp(f_list->filter_descriptor->name, "nl_simple_binocular_q_filter") == 0){
        filter_update (f_list->filter_descriptor);
      }
      if (strcmp(f_list->filter_descriptor->name, "nl_complex_mono_right_filter") == 0){
        filter_update (f_list->filter_descriptor);
      }
      if (strcmp(f_list->filter_descriptor->name, "nl_complex_mono_left_filter") == 0){
        filter_update (f_list->filter_descriptor);
      }
      if (strcmp(f_list->filter_descriptor->name, "nl_complex_binocular_filter") == 0){
        filter_update (f_list->filter_descriptor);
      }
...
void input_generator (INPUT_DESC *input, int status)
{
	...
	if (((input->win == image_left.win) || (input->win == image_right.win)) && (input->win != 0) && (status == MOVE))
	{
		...
		glutSetWindow(input->win);
		robot_input_display ();
//    update_not_simple_mono_cells_filters ();
...
// ----------------------------------------------------------------------------
// ReconstructByDisparityMapLogPolar - Reconstroi o mundo em
// torno do ponto de vergencia atraves do mapa de
// disparidade considerando uma amostragem LOG POLAR.
//
// Entrada: Nenhuma.
//
// Saida: Nenhuma.
...
void ReconstructByDisparityMapLogPolar()
{
        int u, v;
	int w, h, wi, hi, xi, yi;
...
void set_vergence (void)
{
  ...
  // Gets the fartest left point
	...
	nMinScan = (nMinScan < 0) ? 0 : nMinScan;
        printf ("nMinScan = %d\n", nMinScan);
  g_pRangeLine[0] = nMinScan;
  g_pRangeLine[1] = (int) image_right.wh - (p_dblLeftPoint[1] + 0.5) - 1;
	...
	nMaxScan = (nMaxScan >= image_left.ww) ? image_left.ww - 1: nMaxScan;
        printf ("nMaxScan = %d\n", nMaxScan);
  g_pRangeLine[2] = nMaxScan;
  g_pRangeLine[3] = (int) image_right.wh - (p_dblLeftPoint[1] + 0.5) - 1;
...
{
	int nMaxScan, nMinScan, nInfinity;
	int i, j, x, y, nIndex;
	int wo, ho;
	float fltMinSum, fltAux, fltSum;
	time_t start, end;
  ...
  nInfinityDisp = p_dblLeftPoint[0] - p_dblRightPoint[0];//nInfinity - (((double)image_left.wh)/2.0) + 0.5;       
        ...
        if (fltAux >= g_fltDispAux[nIndex])
				{
					...
					{
						g_DispData->neg_slope[nIndex] = 0;
						AddLocalMin(nIndex, g_fltDispAux[nIndex], g_fltConfAux[nIndex], image_left.wxd - image_right.wxd);//image_left.wxd - 1);
						nl_mt_gaussian_map.neuron_vector[nIndex].output.fval = g_fltDispAux[nIndex];
						nl_confidence_map.neuron_vector[nIndex].output.fval = g_fltConfAux[nIndex];
						nl_disparity_map.neuron_vector[nIndex].output.fval = image_left.wxd - image_right.wxd;//image_left.wxd - 1;
					}
				}
				else
					g_DispData->neg_slope[nIndex] = 1;
				...
				g_AllDispData->samples[image_left.wxd - nMinScan][nIndex].val = g_fltDispAux[nIndex];
				g_AllDispData->samples[image_left.wxd - nMinScan][nIndex].conf = g_fltConfAux[nIndex];
				g_AllDispData->samples[image_left.wxd - nMinScan][nIndex].pos = image_left.wxd - image_right.wxd; //image_left.wxd;
			}
    }
...
/*
	...
	for (i = 0; i < (wo * ho); i++)
	{
		nl_disparity_map.neuron_vector[i].output.fval -= nLeft;
		for (j = 0; j < g_nAllSamples; j++)
			g_AllDispData->samples[j][i].pos -= nLeft;
	}
*/
	// Calcula as variacoes das saidas de MT gaussian
	for (i = 0; i < (wo * ho); i++)
...
void Reconstruct (void)
{
  ...
  switch (MAPPING_TYPE){
    case MAP_LOG_POLAR:
      ReconstructByDisparityMapLogPolar();
    break;
    case MAP_PLAN:
      ReconstructByDisparityMapPlan();
    break;
    default:
      printf("MAPPING_TYPE Invalid!\n");
    break;
...
void calculate_disparity (NEURON_LAYER *nlDisparityMap, int wxd)
{
    ...
    switch (MAPPING_TYPE){
      ...
      break;
      case MAP_PLAN:
        nlDisparityMap->neuron_vector[i].output.fval = (float) g_DispData->samples[0][i].pos;
        nl_mt_gaussian_map.neuron_vector[i].output.fval = g_DispData->samples[0][i].val;
        nl_confidence_map.neuron_vector[i].output.fval = g_DispData->samples[0][i].conf;
      break;
      default:
...
//    printf("val = %f, limit = %f\n", nl_mt_gaussian_map.neuron_vector[i].output.fval, limit);
    if (nl_mt_gaussian_map.neuron_vector[i].output.fval > limit){
      nl_disparity_map.neuron_vector[i].output.fval = nInfinityDisp;
...
{	
	...
	switch (key_value[0]) 
	{
      ...
      confiability (0.5);
			...
			glutSetWindow(out_mt_gaussian_map.win);
			update_output_image (&(out_mt_gaussian_map));
			glutPostWindowRedisplay (out_mt_gaussian_map.win);
      break;
		case 'R':
		case 'r':
			Reconstruct ();
