...
void ViewerKeyboard (unsigned char key, int x, int y)
{
	switch (key)
	{
		...
		case 'P':
                        g_pMap->SetPosition(g_pMap->position.x + 10, g_pMap->position.y, g_pMap->position.z);
			break;
		case 'p':
                        g_pMap->SetPosition(g_pMap->position.x - 10, g_pMap->position.y, g_pMap->position.z);
			break;
		case 'O':
                        g_pMap->SetPosition(g_pMap->position.x, g_pMap->position.y + 10, g_pMap->position.z);
			break;
		case 'o':
                        g_pMap->SetPosition(g_pMap->position.x, g_pMap->position.y - 10, g_pMap->position.z);
			break;
		case 'I':
                        g_pMap->SetPosition(g_pMap->position.x, g_pMap->position.y, g_pMap->position.z + 10);
			break;
		case 'i':
                        g_pMap->SetPosition(g_pMap->position.x, g_pMap->position.y, g_pMap->position.z - 10);
			break;
		...
		case 'B':
                        g_pExternalView->SetFixationPoint(g_pExternalView->fixationpoint.x + 10, g_pExternalView->fixationpoint.y, g_pExternalView->fixationpoint.z);
			break;
		case 'b':
                        g_pExternalView->SetFixationPoint(g_pExternalView->fixationpoint.x - 10, g_pExternalView->fixationpoint.y, g_pExternalView->fixationpoint.z);
			break;
		case 'N':
                        g_pExternalView->SetFixationPoint(g_pExternalView->fixationpoint.x, g_pExternalView->fixationpoint.y + 10, g_pExternalView->fixationpoint.z);
			break;
		case 'n':
                        g_pExternalView->SetFixationPoint(g_pExternalView->fixationpoint.x, g_pExternalView->fixationpoint.y - 10, g_pExternalView->fixationpoint.z);
			break;
		case 'M':
                        g_pExternalView->SetFixationPoint(g_pExternalView->fixationpoint.x, g_pExternalView->fixationpoint.y, g_pExternalView->fixationpoint.z + 10);
			break;
		case 'm':
                        g_pExternalView->SetFixationPoint(g_pExternalView->fixationpoint.x, g_pExternalView->fixationpoint.y, g_pExternalView->fixationpoint.z - 10);
			break;
 		case 'z':
		case 'Z':
                        g_pMap->SetPosition(0, 0, 0);
                        g_pExternalView->SetFixationPoint(0, 0, 0);
			break;
                ...
                // Dumps the map
                case 'D':
                        if (!g_pMap->SavePointCloud ("cloud.txt"))
...
int ViewerInitialize (void)
{
        ...
        // Creates the map
	g_pMap = new TMap (g_nMapWidth, g_nMapHeight, -g_fltFovy / (2.0f * g_fltAspectRatio), g_fltFovy / (2.0f * g_fltAspectRatio), g_nMapLayersNumber, g_fltNear, g_fltFar, g_fltViewerTerrainSize);
