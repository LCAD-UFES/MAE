...
#define MIN_CONFIANCE_TO_RETRAIN 30
...
//	map_v1_to_image(&xi, &yi, IMAGE_WIDTH_RESIZED, IMAGE_HEIGHT_RESIZED, u_neuron, v_neuron, w, h, 0, 0, 0, log_factor);
	...
	yi = gt_x_displecement_from_fovea(RED(table_v1.neuron_vector[v_neuron * w + u_neuron].output.ival));	
			...
			{   
				for (u_neighbor_neuron = lim_inf_u_neighbor; u_neighbor_neuron <= lim_sup_u_neighbor; u_neighbor_neuron++)
				{
					if ((u_neighbor_neuron != u_neuron) || (v_neighbor_neuron != v_neuron))
			        {
						...
						map_v1_to_image(&xi_neighbor, &yi_neighbor, IMAGE_WIDTH_RESIZED, IMAGE_HEIGHT_RESIZED, u_neighbor_neuron, v_neighbor_neuron, w, h, IMAGE_WIDTH_RESIZED/2, IMAGE_HEIGHT_RESIZED/2, 0, log_factor);			
						...
						if (BLUE(neighbor_neuron_output) > 0)
						{
							...
							yi_neighbor = gt_x_displecement_from_fovea(RED(table_v1.neuron_vector[v_neighbor_neuron * w + u_neighbor_neuron].output.ival));	
	...
	switch (key = key_value[0])
	{
		...
		case 'd':
			...
			if (count != 0.0)
			{
				...
				{
					if (x/count == (float)(TAM_NL_ZOOM/2 + 80))
						zoom = 0;
					else
					{
