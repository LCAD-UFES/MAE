...
*/	for (v = 0; v < h; v++)
	...
	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			value = nl_input->neuron_vector[u + w * v].output.fval;
//			nl_output->neuron_vector[u + w * v].output.fval = ( value > threshold + 0.5*(max - threshold) ? value : 0.0 );
			nl_output->neuron_vector[u + w * v].output.fval = value;
		}
	}
	...
	for (v = 0; v < hV; v++)
	{
		for (u = 0; u < wU; u++)
		{
			...
			if (value != 0){
				...
				xc = gt_x_displecement_from_fovea(GREEN(value));   
				yc = gt_y_displecement_from_fovea(RED(value));
				xi = gt_x_displecement_from_fovea(GREEN(table_v1.neuron_vector[v * wU + u].output.ival));
				yi = gt_y_displecement_from_fovea(RED(table_v1.neuron_vector[v * wU + u].output.ival));	
...
//				if (ri > 10.0){
					...
					else x = -20.0;//1.0;//(double)rc;

					partial_weight = nl_v1_activation_map_neuron_weight_thresholded.neuron_vector[v * wU + u].output.fval;
					//printf("rc = %f, ri = %f, x = %f conta= %f\n", rc, ri, x, ((x + 10.0)*40.0 + 0.5) );
					//printf("partial_weight = %f\n", partial_weight);

					if ((x >= -10.0) && (x <= 10.0))
						nl_output->neuron_vector[(int)((x + 10.0)*80.0 + 0.5)].output.fval += 1.0 * partial_weight;
						//nl_output->neuron_vector[(int)((x + 10.0)*10.0 + 0.5)].output.fval += 1.0 * partial_weight;
						//printf("nl_output->neuron_vector = %f\n",nl_output->neuron_vector[(int)((x + 10.0)*80.0 + 0.5)].output.fval);
//						
				}
...
void generate_hamming_activation_map(FILTER_DESC *filter_desc)
{
	PARAM_LIST *p_list = NULL;
	NEURON_LAYER_LIST *n_list = NULL;
	NEURON_LAYER *nl_output = NULL, *nl_input = NULL;
	...
	for (yi = 0; yi < hi; yi++)
		for(xi = 0; xi < wi; xi++)
			nl_output->neuron_vector[xi + yi * wi].output.ival =  nl_input->neuron_vector[xi + yi * wi].last_hamming_distance;
}
