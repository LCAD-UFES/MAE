...
int g_vCountPerson[MAX_PERSON_ID-MIN_PERSON_ID+1];
int g_nStatus;
int g_nTries = 1;
int g_nTry = 1;
int g_nFacePart;
...
int GetGroundTruths (void)
{
	...
	sprintf(strFileName, "%s/%s%s.txt", INPUT_PATH, g_chPersonID[g_nPersonID], g_chIllumination[g_nIllumination]);

	if ((pFile = fopen (strFileName, "r")) == NULL)
	{
		printf ("### Error in GetGroundTruths(): cannot open file '%s'.\n", strFileName);
...
void SaveGroundTruths (void)
{
	...
	{
		printf ("### Error in SaveGroundTruths(): cannot open file '%s' to write.\n", strFileName);
		return;
	}
...
* Inputs: strFileName -
*	  nDirection -
* Output: returns 1 if OK, 0 otherwise
***********************************************************
*/
...
int ReadFaceInput(INPUT_DESC *input, char *strFileName)
{
	...
	if ((pFile = fopen (strFileName, "r")) == NULL)
	{
		printf ("### Error in ReadFaceInput(): cannot open file '%s' (ReadFaceInput).\n", strFileName);
		return (-1);
	}
...
void make_input_image_face_recog (INPUT_DESC *input, int w, int h)
{
	...
	switch(TYPE_SHOW)
	{
		...
		default:
			...
			Erro ("### Error in make_input_image_face_recog(): Invalid Type Show ", message, "");
			return;
	}
...
int 
GetNewFace(INPUT_DESC *input, int nDirection)
{
	char strFileName[128];
	...
	if (!GetNextFileName(strFileName, nDirection))
	{
		printf ("### Error in GetNewFace(): Could not get next filename.\n");
		return (-1);
	}
