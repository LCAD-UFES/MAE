...
void ReconstructByDisparityMap()
{
	...
	for (v = 0; v < h; v++)
	{
		for (u = nStartCol; u < nEndCol; u++)
		{
			...
			if (nl_disparity_map.output_type == GREYSCALE_FLOAT)
...
// ----------------------------------------------------------------------------
SAMPLES 
best_neig (SAMPLES *sample, int x, int y, int w, int h)
{
	SAMPLES best_n, current_n;
	float conf = 0.0;

	if (x-1 >= 0)
	{
		current_n = sample[y*w + x-1];
		if (current_n.conf > conf)
		{
			conf = current_n.conf;
			best_n = current_n;
		}
...
//			if ((dist > 5) && (g_DispData->samples[0][y * w + x].conf < 0.0003))
//			{
//				g_DispData->samples[0][y * w + x] = best_neig (g_DispData->samples[0], x, y, w, h);
//			}
//			else
//			{
