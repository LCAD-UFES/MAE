...
contingency_table(int class_id, int wnn_predicted_max_min)
{
		...
		if(g_data_set[g_current_sample + 2].max_min == class_id)
		{
...
buy ()
{
	g_amount_stocks = (g_money / g_data_set[g_current_sample+2].sample_price);
	g_money = 0;
	g_current_price = g_data_set[g_current_sample+2].sample_price;
	printf ("    buy amount_stocks = %.2f\n",g_amount_stocks);
}

void
sell ()
{
	g_money = (g_amount_stocks * g_data_set[g_current_sample+2].sample_price);
	g_amount_stocks = 0;
	g_current_price = g_data_set[g_current_sample+2].sample_price;
	printf("     sell money = %.2f\n",g_money);
}
...
buy_or_sell (int wnn_predicted_max_min)
{
	if (wnn_predicted_max_min == 1 && g_money > 0 ){
		printf("predictor= %i\n", wnn_predicted_max_min);
		buy ();
	}
	else if (wnn_predicted_max_min == 2 && g_amount_stocks > 0 ){
		printf("predictor= %i\n", wnn_predicted_max_min);
		sell ();
	}
}
...
base_output_handler_contingency_table (OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state, int (*EvaluateOutput) (OUTPUT_DESC *, int *))//
{
	...
	if (g_nStatus == RECALL_PHASE)
	{
		...
		g_data_set[g_current_sample+2].wnn_predicted_max_min = wnn_predicted_max_min;
		...
		for (x = (g_day_wnn_predicted * IF_WIDTH) ; x < (g_day_wnn_predicted + 1)*IF_WIDTH ; x++)
		{
			if(g_data_set[x].wnn_predicted_max_min == 0 ||g_data_set[x].wnn_predicted_max_min == 1 ||g_data_set[x].wnn_predicted_max_min == 2)
			{
				...
				else if (g_data_set[x].wnn_predicted_max_min == 1) // if mininum point
				{
					...
					/*for(k = 0- g_day_wnn_predicted; k < IF_HEIGHT; k++) // Draws a vertical blue line
					{
				...
				else if (g_data_set[x].wnn_predicted_max_min == 2) // if maximum point
				{
					...
					/*for(k = 0- g_day_wnn_predicted; k < IF_HEIGHT; k++) // Draws a vertical green line
					{
...
void inflection_points_generator (INPUT_DESC *input, int status)
{
	...
	else
	{
		...
		for (x = (g_day_inflection_point * IF_WIDTH) ; x < (g_day_inflection_point + 1)*IF_WIDTH ; x++)
		{
			...
			{
				neuron[y * x_w + x].output.ival = color << 16; // color = blue
				/*for(k = 0 - g_day_inflection_point; k < IF_HEIGHT; k++) // Draws a vertical blue line
				{
					neuron[k * x_w + x].output.ival = color << 16;
				}*/
			}
			...
			{
				neuron[y * x_w + x].output.ival = color << 8; // color = green
				/*for(k = 0 - g_day_inflection_point; k < IF_HEIGHT; k++) // Draws a vertical green line
				{
					neuron[k * x_w + x].output.ival = color << 8;
				}*/
			}
...
void draw_output (char *strOutputName, char *strInputName)//
{
	...
	{
		neuron[i].output.ival = g_data_set[g_current_sample + 2].max_min;	
	}
	
	if (g_data_set[g_current_sample].train == 1)
	{
		if(g_data_set[g_current_sample + 2].max_min == 0){
			g_num_neutral_train++;
		}
		if(g_data_set[g_current_sample + 2].max_min == 1){
			g_num_min_train++;
		}
		if(g_data_set[g_current_sample + 2].max_min == 2){
			g_num_max_train++;
		}
	}
