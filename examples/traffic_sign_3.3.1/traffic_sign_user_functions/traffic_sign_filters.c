...
/*
*********************************************************************************
* Function: dead_band_noise_mask_nl_filter					*
* Description: 			                        			*
* Inputs: 				                  			*
* Output: 				                			*
*********************************************************************************
...
void dead_band_noise_mask_nl_filter (FILTER_DESC *filter_desc)
{
	PARAM_LIST *p_list = NULL;
	NEURON_LAYER_LIST *n_list = NULL;
	NEURON_LAYER *nl_output = NULL, *nl_input = NULL;
	...
	if (p_number != 1)
	{
		Erro ("Error: Wrong number of parameters. The dead_band_noise_mask_nl_filter must have no parameters.", "", "");
		return;
	}
	...
	// Parallel filtering capabilities where OpenMP available
#ifdef	_OPENMP
	#pragma omp parallel for private(yo,xo)
#endif	
	for (yo = 0; yo < ho; yo++)
		for (xo = 0; xo < wo; xo++)
			//nl_output->neuron_vector[xo + yo * wo].output.ival = NEURON_FULL_MASK_GREYSCALE(nl_input->neuron_vector[xo + yo * wo].output.ival);
			nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00f0f0f0 & nl_input->neuron_vector[xo + yo * wo].output.ival;
			//nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00e0e0e0 & nl_input->neuron_vector[xo + yo * wo].output.ival;
			//nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00c0c0c0 & nl_input->neuron_vector[xo + yo * wo].output.ival;
			//nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00808080 & nl_input->neuron_vector[xo + yo * wo].output.ival;

			//nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00f8f8f8 & nl_input->neuron_vector[xo + yo * wo].output.ival;
			//nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00fcfcfc & nl_input->neuron_vector[xo + yo * wo].output.ival;
			//nl_output->neuron_vector[xo + yo * wo].output.ival = 0x00fefefe & nl_input->neuron_vector[xo + yo * wo].output.ival;
			//nl_output->neuron_vector[xo + yo * wo].output = nl_input->neuron_vector[xo + yo * wo].output;
}

/*
*********************************************************************************
* Function: minimum_energy_channel_filter					*
* Description: 			                        			*
* Inputs: 				                  			*
* Output: 				                			*
*********************************************************************************
...
void minimum_energy_channel_filter (FILTER_DESC *filter_desc)
{
	...
	// Gets the Output Layer dimensions	
	wo = nl_output->dimentions.x;
	ho = nl_output->dimentions.y;
	...
	for (yo = 0; yo < ho; yo++)
		for (xo = 0; xo < wo; xo++)
		{
			red_total += RED(nl_input->neuron_vector[xo + yo * wo].output.ival);
			green_total += GREEN(nl_input->neuron_vector[xo + yo * wo].output.ival);
			blue_total += BLUE(nl_input->neuron_vector[xo + yo * wo].output.ival);
		}

	if( red_total < green_total && red_total < blue_total )
	{
		for (yo = 0; yo < ho; yo++)
			for (xo = 0; xo < wo; xo++)
				nl_output->neuron_vector[xo + yo * wo].output.ival = RED(nl_input->neuron_vector[xo + yo * wo].output.ival);
	}
	else if( blue_total < red_total && blue_total < green_total )
	{
		for (yo = 0; yo < ho; yo++)
			for (xo = 0; xo < wo; xo++)
				nl_output->neuron_vector[xo + yo * wo].output.ival = BLUE(nl_input->neuron_vector[xo + yo * wo].output.ival);
	}
	else
	{
		for (yo = 0; yo < ho; yo++)
