...
{
	MAE_FSCANF_WRAPPER(fscanf(g_signs_data_file, "%05d_%05d_%05d %d %d %d %d %d %d\n", &g_signUniqueID, &g_signID, &g_sign_frameID, &g_img_w, &g_img_h, &g_roi_x1, &g_roi_y1, &g_roi_x2, &g_roi_y2),"get_first_sign_id")
	return 0;
}
...
get_file_lines_len (char *file)
{
	...
	MAE_SYSTEM_WRAPPER(system(command),"get_file_lines_len")
	linhas = fopen("linhas.deleteme", "r");
	MAE_FSCANF_WRAPPER(fscanf(linhas, "%d %*s\n", &len),"get_file_lines_len")
	MAE_SYSTEM_WRAPPER(system("rm linhas.deleteme"),"get_file_lines_len")
...
int EvaluateOutput2(OUTPUT_DESC *output, float *confidence)
{
	...
	for (i = 0; i < output->wh; i++)
	{
		for (j = 0; j < output->ww; j++)
		{
			...
			{
				gaussian_value = 1.0 * normal_dist((float)j, (float)(output->wh/2.0), VOTE_VARIANCE);

				sign_count[neuron_vector[i*output->wh + j].output.ival] += gaussian_value;
