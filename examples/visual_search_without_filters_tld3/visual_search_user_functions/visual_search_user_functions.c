...
move_input(INPUT_DESC *input)
{
	...
	else
	{
		...
		all_neurons_update();
		filter_update(get_filter_by_name("nl_v1_activation_map_red_f_filter"));
		all_outputs_update();
	}
}
...
/*
double
compute_confidence_very_old(NEURON *n, int g_halph_band_width, int w, int h)
{
...
{
	double count, confidence, xy_conf;
	int u, v;
	int xi, yi;
	int ground_truth_w;
	int ground_truth_h;
...
*/
			if ( nl_v1_activation_map_red_confidence.neuron_vector[v * w + u].output.fval > 0 ){
//				confidence += 2*fabs(n[v * w + u].output.fval - nl_v1_activation_map_red_confidence.neuron_vector[v * w + u].output.fval);
//				confidence += fabs(nl_v1_activation_map_green.neuron_vector[v * w + u].output.fval - nl_v1_activation_map_green_confidence.neuron_vector[v * w + u].output.fval);
//				confidence += fabs(nl_v1_activation_map_blue.neuron_vector[v * w + u].output.fval - nl_v1_activation_map_blue_confidence.neuron_vector[v * w + u].output.fval);

				if (n[v * w + u].output.fval > 0.0){
					xy_conf = fabs(nl_v1_activation_map_green.neuron_vector[v * w + u].output.fval - nl_v1_activation_map_green_confidence.neuron_vector[v * w + u].output.fval);
				    xy_conf += fabs(nl_v1_activation_map_blue.neuron_vector[v * w + u].output.fval - nl_v1_activation_map_blue_confidence.neuron_vector[v * w + u].output.fval);
					if ( n[v * w + u].output.fval == nl_v1_activation_map_red_confidence.neuron_vector[v * w + u].output.fval
						 && xy_conf < 5	)
					{
						confidence += 3.0;
						count += 2.0;
					}
					else if (n[v * w + u].output.fval == nl_v1_activation_map_red_confidence.neuron_vector[v * w + u].output.fval
								|| xy_conf < 5)
					{
						confidence += 2.0;
						count += 1.0;
	...
	//confidence = 1/(1+confidence)*3000000;
	//printf("CONFIDENCE = %f\n", confidence / count);
...
output_handler_weighted_average_value_position_old(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	g_confidence = compute_confidence(out_v1_activation_map_red_f.neuron_vector, g_halph_band_width, w, h);
...
output_handler_weighted_average_value_position(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	g_confidence = compute_confidence(nl_v1_activation_map_red.neuron_vector, g_halph_band_width, w, h);
...
output_handler_max_value_position(OUTPUT_DESC *output, int type_call, int mouse_button, int mouse_state)
{
	...
	unfiltered_activation_map = nl_v1_activation_map_red.neuron_vector;
	g_confidence = 0.0;
...
saccade(INPUT_DESC *input)
{
	...
	do
	{
		...
		count++;
			...
			y = nl_target_coordinates.neuron_vector[1].output.fval;
		#endif
	} while (((fabs(x) > 0.5) || (fabs(y) > 0.5)) && (count < 5));
...
void
init_nl_linear_distance(NEURON_LAYER *nl_landmark_eval_mask)
{
	int w, h, u, v;
	NEURON *neuron_vector;
	neuron_vector = nl_landmark_eval_mask->neuron_vector;
	...
	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			neuron_vector[v * w + u].output.fval = fabs(u - w/2.0)*255.0/w;
		}
	}
}

void
init_nl_linear_angle(NEURON_LAYER *nl_landmark_eval_mask)
{
	int w, h, u, v, i, xi, yi, xW, yH;
	NEURON *neuron_vector;
	neuron_vector = nl_landmark_eval_mask->neuron_vector;

	w = nl_landmark_eval_mask->dimentions.x;
	h = nl_landmark_eval_mask->dimentions.y;
	xW = in_pattern_filtered_translated_temp.dimentions.x;
	yH = in_pattern_filtered_translated_temp.dimentions.y;

	for (i = 0; i < xW*yH; i++)
		in_pattern_filtered_translated_temp.neuron_vector[i].output.fval = 0.0;

	for (v = 0; v < h; v++)
	{
		for (u = 0; u < w; u++)
		{
			map_v1_to_image(&xi, &yi, IMAGE_WIDTH_RESIZED, IMAGE_HEIGHT_RESIZED, u, v, w, h, (double) xW / 2.0, (double) yH / 2.0, (double) h / (double) (h - 1), LOG_FACTOR);
			if ( in_pattern_filtered_translated_temp.neuron_vector[yi * xW + xi].output.fval == 0.0 )
			{
				neuron_vector[v * w + u].output.fval = v*255.0/h;
				in_pattern_filtered_translated_temp.neuron_vector[yi * xW + xi].output.fval = neuron_vector[v * w + u].output.fval;
			} 
			else
			{
				neuron_vector[v * w + u].output.fval = in_pattern_filtered_translated_temp.neuron_vector[yi * xW + xi].output.fval;
			}
		}
	}
}
...
void
init_nl_linear_x_y(NEURON_LAYER *nl_landmark_eval_mask_x, NEURON_LAYER *nl_landmark_eval_mask_y)
{
	int w, h, u, v, i, xi, yi, xW, yH;
	NEURON *neuron_vector_x;
	NEURON *neuron_vector_y;
	neuron_vector_x = nl_landmark_eval_mask_x->neuron_vector;
	neuron_vector_y = nl_landmark_eval_mask_y->neuron_vector;

	w = nl_landmark_eval_mask_x->dimentions.x;
	h = nl_landmark_eval_mask_x->dimentions.y;
	xW = in_pattern_filtered_translated.dimentions.x;
	yH = in_pattern_filtered_translated.dimentions.y;
 	...
 	for (v = 0; v < h; v++)
	{
		...
		{
			map_v1_to_image(&xi, &yi, IMAGE_WIDTH_RESIZED, IMAGE_HEIGHT_RESIZED, u, v, w, h, (double) xW / 2.0, (double) yH / 2.0, (double) h / (double) (h - 1), LOG_FACTOR);
			neuron_vector_x[v * w + u].output.fval = xi * 255.0/xW;
			neuron_vector_y[v * w + u].output.fval = yi * 255.0/yH;
		}
	}
}
...
train_visual_search()
{
	...
	init_nl_landmark_eval_mask_for_log_polar(&nl_v1_activation_map_red);
	init_nl_landmark_eval_mask_for_log_polar(&nl_v1_activation_map_red_confidence);
//	init_nl_linear_angle(&nl_v1_activation_map_green);
//	init_nl_linear_distance(&nl_v1_activation_map_blue);
	init_nl_linear_x_y(&nl_v1_activation_map_green, &nl_v1_activation_map_blue);
	init_nl_linear_x_y(&nl_v1_activation_map_green_confidence, &nl_v1_activation_map_blue_confidence);

	num_neurons = get_num_neurons (nl_v1_activation_map_red.dimentions);
	neuron_vector = nl_v1_activation_map_red.neuron_vector;

//	train_neuron_layer("nl_v1_activation_map");
	for (i = 0; i < num_neurons; i++)
		if ( neuron_vector[i].output.fval >= 0 )
		{
			train_neuron(&nl_v1_activation_map_red, i);
			train_neuron(&nl_v1_activation_map_green, i);
			train_neuron(&nl_v1_activation_map_blue, i);
		}
...
flush_and_retraining(PARAM_LIST *pParamList)
{
	...
	printf("retreinando!!!!\n");
	clear_neural_layers_memory ("nl_v1_activation_map_red");	//clear neural layer memory
	train_visual_search();
	output.ival = 0;
	return(output);
}
...
f_keyboard(char *key_value)
{
	...
	switch (key = key_value[0])
	{
		...
		case 'f':
			...
			clear_neural_layers_memory("nl_v1_activation_map_red");
			break;
	}
...
ForwardVisualSearchNetwork(PARAM_LIST *pParamList)
{
	...
	forward_objects("nl_v1_activation_map_red");
	forward_objects("nl_v1_activation_map_green");
	forward_objects("nl_v1_activation_map_blue");
	forward_objects("nl_v1_activation_map_red_f_filter");
	all_outputs_update();
