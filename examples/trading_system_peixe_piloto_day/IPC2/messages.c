...
build_quotes_prices(qsor_quotes_message *quotes, char *quotes_prices)
{
	...
	quotes->price = (float *) malloc(quotes->num_stocks * sizeof(float));
...
build_quotes_volumes(qsor_quotes_message *quotes, char *quotes_volumes)
{
	...
	quotes->volume = (float *) malloc(quotes->num_stocks * sizeof(float));
...
void 
free_state(ts_state_message* state)
{
	free(state->state_name);
	free(state->date_and_time);
	free(state->host);
	free(state);
}
