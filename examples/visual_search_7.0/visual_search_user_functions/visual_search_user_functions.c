...
#define NL_SAMPLES_NAME			"nl_samples"
...
memorize_features_vector ()
{
	...
	static NEURON_LAYER *nl_samples  = NULL;
	static NEURON_LAYER *nl_target  = NULL;
	static int index = 0;
	int i, j, features_number, samples_number;
	float value;
	
	if (nl_features == NULL)
	{
		nl_features = get_neuron_layer_by_name (NL_FEATURES_NAME);
		nl_samples  = get_neuron_layer_by_name (NL_SAMPLES_NAME);
		nl_target   = get_neuron_layer_by_name (NL_TARGET_NAME);
	}
	
	features_number = nl_samples->dimentions.x;
	samples_number  = nl_samples->dimentions.y;

	for (i = 0; i < features_number; i++)
		nl_samples->neuron_vector[i + index * features_number] = nl_features->neuron_vector[i];
	
	index = (index + 1) % samples_number;
	
	for (i = 0; i < 2 * features_number; i++)
		nl_target->neuron_vector[i].output.fval = 0.0;
		
	for (j = 0; j < samples_number; j++)
	{
		for (i = 0; i < features_number; i++)
		{
			value = nl_samples->neuron_vector[i + j * features_number].output.fval;
			nl_target->neuron_vector[i].output.fval += value;
			nl_target->neuron_vector[i + features_number].output.fval += value * value;
		}
	}

	for (i = 0; i < features_number; i++)
	{
		nl_target->neuron_vector[i].output.fval /= samples_number;
		nl_target->neuron_vector[i + features_number].output.fval -= samples_number * nl_target->neuron_vector[i].output.fval * nl_target->neuron_vector[i].output.fval;
		nl_target->neuron_vector[i + features_number].output.fval /= (samples_number - 1);
	}
