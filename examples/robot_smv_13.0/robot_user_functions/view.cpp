...
void TView::RotateAroundFixationPoint (float fltDeltaAlpha, float fltDeltaBeta)
{
        ...
        if (fltAlpha > (2.0 * M_PI))
		fltAlpha = fltAlpha - 2.0 * M_PI;
	else if (fltAlpha < (-2.0 * M_PI))
		fltAlpha = 2.0 * M_PI + fltAlpha;

        if (fltBeta > (2.0 * M_PI))
		fltBeta = fltBeta - 2.0 * M_PI;
	else if (fltBeta < (-2.0 * M_PI))
		fltBeta = 2.0 * M_PI + fltBeta;
...
// ----------------------------------------------------------------------
// TView::GetPosition - gets the view position
//
// Inputs: none
//
// Outputs: (fltX, fltY, fltZ) - the current view position coordinates
// ----------------------------------------------------------------------

void TView::GetPosition (float *fltX, float *fltY, float *fltZ)
{
	*fltX = position.x;
	*fltY = position.y;
	*fltZ = position.z;
