...
double line_fit_slope (vector<double>& x, vector<double>& y)
{
	trace("line_fit_slope", string() + x + ", " + y);

	int n = Primitives::min(x.size(), y.size());
	int i = 0;
	double* array_x = (double*) malloc(n * sizeof(double));
	double* array_y = (double*) malloc(n * sizeof(double));
	for (i = 0; i < n; i++)
	{
		array_x[i] = x[i];
		array_y[i] = y[i];
	...
	trace("gsl_fit_linear", string() + x + ", 1, " + y + ", 1, " + n + ", &c0, &c1, &cov00, &cov01, &cov11, &sumsq");
	gsl_fit_linear(
		array_y, 1, array_x, 1, n,
		&c0, &c1, &cov00, &cov01, &cov11,
		&sumsq);
		
	double m = 1 / (isnan(c1) ? PI_HALF : c1);

	log(string() + "LINE{slope(" + m + ")}");

	return m;
}

LINE *
line_fit (double x0, double y0, double z0, vector<double>& x, vector<double>& y)
{
	trace("line_fit", string() + x0 + ", " + y0 + ", " + z0 + ", " + x + ", " + y + "");

	double m = line_fit_slope(x, y);
	double norm = sqrt(1 + POW2(m));
