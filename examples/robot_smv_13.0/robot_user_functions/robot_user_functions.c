...
compute_working_area (void)
{
	...
	robot_smv_save_working_area(0, g_pWorkingArea);
}
...
robot_input_mouse (int button, int state, int x, int y)
{
	...
	GLint real_y = viewport[3] - (GLint) y - 1;

	GLdouble mvmatrix[16], projmatrix[16];
	glGetDoublev (GL_MODELVIEW_MATRIX, mvmatrix);
	glGetDoublev (GL_PROJECTION_MATRIX, projmatrix);

	INPUT_DESC *input = get_input_by_win(glutGetWindow());
	...
	switch (button)
	{
		case GLUT_LEFT_BUTTON:
			...
			else if (state == GLUT_UP)
			{
				...
				input->valid = 0;
				GLdouble wxu = (GLint) (wx + 0.5);
				GLdouble wyu = (GLint) (wy + 0.5);
...
/*		int wi = nl_simple_mono_left.dimentions.x;
		...
		else
		{
			...
			RECEPTIVE_FIELD_DESCRIPTION *receptive_field = nl_simple_mono_right_filter->private;
...
f_keyboard (char *key_value)
{
	...
	{
		case 'b':
			robot_smv_select_half(1);
		break;
		case 'c':
			robot_smv_compute_model_3d_volume();
		break;
		case 'd':
			robot_smv_compute_point_cloud_current();
		break;
		case 'e':
			ViewerEraseMap ();
		break;
		case 'f':
			robot_smv_select_half(0);
		break;
		case 'h':
		{
			int halves = robot_smv_toggle_two_halves();
			printf("Log pile type changed to %s\n", halves < 2 ? "3m" : "6m");
		}
		break;
		case 'R':
			ViewerEraseMap ();
...
//			add_subpixel_disparity_to_disparity_map_plan();
		...
		case 'u':
			...
			break;
		case 'v':
			robot_smv_compute_current();
		break;
		case 'V':
			robot_smv_compute_all();
		break;
		// Zoom out
		case 'z':
			g_fltZoom *= 0.5;
		...
		case 'y':
			...
			printf ("%lf %lf %lf\n", p_dblRightWorldPoints[3],
						 p_dblRightWorldPoints[4],
						 p_dblRightWorldPoints[5]);

			printf("Distance = %3f\n",
				sqrt(
					POW2(p_dblRightWorldPoints[0] - p_dblRightWorldPoints[3]) +
					POW2(p_dblRightWorldPoints[1] - p_dblRightWorldPoints[4]) +
					POW2(p_dblRightWorldPoints[2] - p_dblRightWorldPoints[5])
				)
			);
		...
		case '0':
			robot_smv_select_side(0);
		break;
		case '1':
			robot_smv_select_side(1);
		break;
		case '2':
			robot_smv_select_side(2);
		break;
/*		case 's':
