...
ROBOT_SOCKET g_SocketLeft, g_SocketRight;
...
// Saida: Nenhuma
// ----------------------------------------------------------------------------
void GetImage(INPUT_DESC *input, ROBOT_SOCKET *s)
{
...
int init_user_functions()
{
	...
	//DEPRECATED: old camera calibration method
	LoadCameraParams();

	//LoadCameraParams("Calibration_100810.xml");
...
void input_generator (INPUT_DESC *input, int status)
{
	...
	#ifdef WINDOWS
		dblTempo1 = (double)tv1 / CLOCKS_PER_SEC;
	#else
	...
	#ifdef WINDOWS
		dblTempo2 = (double)tv2 / CLOCKS_PER_SEC;
	#else
...
{
	ImagePoint principalPoint;
	ImagePoint FOV;
	IMAGE_COORDINATE leftPrincipalPoint;
	IMAGE_COORDINATE rightPrincipalPoint;
	double leftCameraFocus;
	double rightCameraFocus;
	double aspectRatio;
	double baseline;

	baseline = GetStereoBaseline();

	GetCameraParameters(&FOV, &principalPoint, &leftCameraFocus, &aspectRatio, LEFT_CAMERA);
	leftPrincipalPoint.x = principalPoint.x;
	leftPrincipalPoint.y = principalPoint.y;
	
	GetCameraParameters(&FOV, &principalPoint, &rightCameraFocus, &aspectRatio, RIGHT_CAMERA);
	rightPrincipalPoint.x = principalPoint.x;
	rightPrincipalPoint.y = principalPoint.y;
...
void
forward_gabor_filters()
{
	FILTER_DESC *filter;

	filter = get_filter_by_name ("nl_translated_image_left_filter");
	filter_update (filter);

// 	filter = get_filter_by_name ("nl_simple_mono_right_filter");
// 	filter_update (filter);
// 	filter = get_filter_by_name ("nl_simple_mono_right_q_filter");
// 	filter_update (filter);
// 
// 	filter = get_filter_by_name ("nl_simple_mono_left_filter");
// 	filter_update (filter);
// 	filter = get_filter_by_name ("nl_simple_mono_left_q_filter");
// 	filter_update (filter);
}


void
forward_mt_and_gassian_filters()
{
	FILTER_DESC *filter;

	filter = get_filter_by_name ("nl_simple_binocular_filter");
	filter_update (filter);
	filter = get_filter_by_name ("nl_simple_binocular_q_filter");
	filter_update (filter);

	filter = get_filter_by_name ("nl_complex_mono_right_filter");
	filter_update (filter);
	filter = get_filter_by_name ("nl_complex_mono_left_filter");
	filter_update (filter);

	filter = get_filter_by_name ("nl_complex_binocular_filter");
	filter_update (filter);

	filter = get_filter_by_name ("nl_mt_filter");
	filter_update (filter);
	filter = get_filter_by_name ("nl_mt_gaussian_filter");
	filter_update (filter);
}


int g_nCurrDisparity = 0;
int setverg_cont = 0;
...
/*	struct timeval start;
	...
	for (image_left.wxd = nMinScan; image_left.wxd < nMaxScan; image_left.wxd++)
	{
		g_nCurrDisparity = image_left.wxd;
		//all_outputs_update();
		//forward_gabor_filters();
		filter_update(v1_mt_filter);
		setverg_cont = 1;
