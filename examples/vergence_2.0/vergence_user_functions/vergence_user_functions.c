		...
		int mouse_state __attribute__ ((unused)) )
{
	...
	else
	{
		lower_x_lim = (w/8)*3;
		upper_x_lim = w - (w/8)*3;
//		lower_x_lim = (w/2) - BAND_WIDTH/2;
//		upper_x_lim = (w/2) + BAND_WIDTH/2;
	}
...
run_train(PARAM_LIST *pParamList)
{
	...
	set_input_layer_translation(&in_vergence_current, in_vergence_current.wxd, in_vergence_current.wyd);
...
run_test(PARAM_LIST *pParamList)
{
	...
	output.ival = in_vergence_current.wxd - in_vergence_trained.wxd;
...
NEURON_OUTPUT
read_points(PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;
	int x, y, i = 0;
	int count = 0;
	...
	output.ival = -1;
	if (pParamList && pParamList->next)
	{
		copy_without_trailings(&filename[0], pParamList->next->param.sval);

		if (pParamList->next->next)
		{
			count = pParamList->next->next->param.ival;
			corners = (POINT *) malloc(count * sizeof(POINT));
		}

		if ((fp = fopen(filename, "r")) == NULL)
		{
			Erro("Could not open file: ", filename, "");
		}
		...
		while(!feof(fp) && (i < count))
		{
			...
			if (sscanf(line, "%d, %d", &x, &y) < 0)
				continue;

			corners[i].x = x;

			corners[i].y = IMAGE_HEIGHT - y;
...
NEURON_OUTPUT
get_point_x(PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;

	if (pParamList && pParamList->next)
		output.ival = corners[pParamList->next->param.ival].x;
	else
		output.ival = -1;
...
NEURON_OUTPUT
get_point_y(PARAM_LIST *pParamList)
{
	NEURON_OUTPUT output;

	if (pParamList && pParamList->next)
		output.ival = corners[pParamList->next->param.ival].y;
	else
		output.ival = -1;
