...
//Draw cylinders as lines in image right
int g_pLineDrawCylinders = 1;

CYLINDER_LIST *cylinders = NULL;
...
void 
draw_cylinder_as_lines_in_input_right(CYLINDER_LIST * cylinder_list)
{
	CYLINDER cylinder;
	...
	while (!is_cylinder_list_out (cylinder_list))
	{
		...
		//printf("cilindro com os dados: %f %f %f %f \n", 
		//cylinder.x_init_cylinder, cylinder.y_init_cylinder,cylinder.x_end_cylinder, cylinder.y_end_cylinder);
		glBegin(GL_LINES);
			glVertex2f (cylinder.x_init_cylinder, cylinder.y_init_cylinder);
			glVertex2f (cylinder.x_end_cylinder, cylinder.y_end_cylinder);
		glEnd ();
		cylinder_list_move_next(cylinder_list);
	}
	glutSwapBuffers ();
	image_right.waiting_redisplay = 0;
}
...
robot_input_display (void)
{
	...
	//Draw cylinders
        if (strcmp(input->name, image_right.name) == 0 && g_pLineDrawCylinders && cylinders != NULL)
        {
		glColor3f (1.0, 0.0, 0.0);
		//printf("desenhando as linhas em image_right. \n");
		draw_cylinder_as_lines_in_input_right(cylinders);
        }
...
f_keyboard (char *key_value)
{
	...
	switch (key)
	{
		...
		break;
		case 'h':
			printf("Halves offset: %ld pixels\n", compute_horizontal_offset());
		break;
		case 'R':
			ViewerEraseMap ();
			Reconstruct ();
