...
void model_3d_fill_face(MODEL_3D *model, int index, int points, double *leftPoints, double *rightPoints, int *colors)
{
	...
	string mode = ConfigDefault::getSetting("faces.mode");
	if (compute_mass_center && mode == "maintenance")
...
CYLINDER_LIST *model_3d_all_cylinders(MODEL_3D *model)
{
	...
	for (int i = NUM_CAMERA_PAIRS - 1, step = 1; i >= 0; i--, step = -1)
		model_3d_face_copy_cylinders(&(faces[i]), list, step);
...
void model_3d_stereo_volume(MODEL_3D *model)
{
	...
	model->height = compute_average_height(face_left, face_top, face_right);
	model->width  = compute_average_width(face_left, face_right);
	model->length = compute_average_length(face_left, face_top, face_right);

	cylinder_list_free(face_left);
	cylinder_list_free(face_top);
	cylinder_list_free(face_right);

	// O volume estéreo, em metros cúbicos.
	model->stereoVolume = model->length * model->width * model->height / pow(10, 9);

	printf("--- Calculo do Volume Estereo ---\n");
	printf("Comprimento: %lf\nAltura: %lf\nLargura: %lf\n", model->length, model->height, model->width);
	printf("Volume estereo: %lf\n\n", model->stereoVolume);
}
