...
[2006.02.07 - Helio Perroni Filho] Converting metric scale to milimeters.
*/
...
random_val_within_range (double range_init, double range_end)
{
	...
	random_val = ((double) rand () / (double) RAND_MAX) * (range_end - range_init) + range_init;
...
generate_initial_circle_list (CYLINDER_LIST *list)
{
	...
	config_load();
	double cylinder_delta_x = config_get_double("circle.packing.delta.x");
	double cylinder_delta_y = config_get_double("circle.packing.delta.y");
	double scale = config_get_double("circle.packing.scale");
	double theta = config_get_double("circle.packing.theta");
...
//	         pi * (cylinder->diameter / 2.0) * (cylinder->diameter / 2.0) * cylinder->size);
	return (pi * (cylinder->diameter / 2.0) * (cylinder->diameter / 2.0) * cylinder->size);
}
...
void 
circle_packing_execute (CYLINDER_LIST *list)
{	
	double volume;
	
	volume = volume_solido (list, 1);
	printf ("Volume solido: %f m3\nNumero de toras: %d\n",
		volume / (1000.0*1000.0), global_num_new_cylinders + global_initial_num_cylinders);
...
circles_f_keyboard (unsigned char key, int x, int y)
{
	...
	{
		case 'v':
			circle_packing_execute (global_cylinder_list);
		break;
	}
}
