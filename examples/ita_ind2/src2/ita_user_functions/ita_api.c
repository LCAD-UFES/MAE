...
DLLEXPORT int MaeUpdateImage (int *p_nNewImage, int nImageWidth, int nImageHeight)
{
	...
	if ((nImageWidth != ita.ww) || (nImageHeight != ita.wh))
	{
	...
	for (j = 0; j < nImageHeight; j++)
	{
        	for (i = 0; i < nImageWidth; i++)
        	{
		      ...
		      ita.image[3 * (i + j * ita.tfw) + 0] = (GLubyte) RED(nPixel);
		      ita.image[3 * (i + j * ita.tfw) + 1] = (GLubyte) GREEN(nPixel);
		      ita.image[3 * (i + j * ita.tfw) + 2] = (GLubyte) BLUE(nPixel);
        	}
	}

	// Updates the input neurons
	ita.up2date = 0;
	update_input_neurons (&ita);
	...
	{
		FILE *pImageFile = fopen (ita.name, "w");
		...
		for (j = nImageHeight; j >= 0 ; j--)
        		for (i = 0; i < nImageWidth; i++)
       			      fprintf (pImageFile, "%d %d %d\n", ita.image[3 * (i + j * ita.tfw) + 0],
								 ita.image[3 * (i + j * ita.tfw) + 1],
								 ita.mage[3 * (i + j * ita.tfw) + 2]);
		fclose (pImageFile);
	}*/
	return (0);
}
