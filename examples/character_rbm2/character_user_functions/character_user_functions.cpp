...
#define SET_AND_TRAIN	5
#define	UNLOAD_LEARNING	6
#define RELOAD_LEARNING	7
#define UPDATE_INPUT	8
#define	BACK_PROP_INIT	9
#define BACK_PROP_TRAIN	10
...
struct fann *g_DBN = NULL;
int g_previous_neurons = 0;
int g_previous_connections = 0;
double g_input_data [60000*28*28];
double g_output_data[10000*28*28];
...
//Extracted from here http://leenissen.dk/fann/forum/viewtopic.php?t=105
struct fann_train_data *
fann_read_train_from_array(fann_type *din, fann_type *dout, unsigned int num_data, unsigned int num_input, unsigned int num_output)
{
	unsigned int i, j;
...
rbm_filter(FILTER_DESC *filter_desc)
{
	...
	{
		network = new BoltzmannMachine(wi * hi, wo * ho);
	...
	if (g_state == READ)
	{
		...
		for (yi = 0; yi < hi; yi++)
		{
			for (xi = 0; xi < wi; xi++)
			{
				...
				{
					data[yi * wi + xi] = nl_input->neuron_vector[yi * wi + xi].output.fval;
				}
				else
				{
		...
		network->read_data(data, wi * hi);

		if (strcmp(filter_desc->name, "character_rbm1_filter") == 0)
		{
			for (yi = 0; yi < hi; yi++)
				for (xi = 0; xi < wi; xi++)
					g_input_data[g_cont_char * wi * hi + yi * wi + xi] = ( ((float)nl_input->neuron_vector[yi * wi + xi].output.ival) / 255.0);

			for (int i = 0; i < 10; i++)
				g_output_data[g_cont_char * 10 + i] = ( i == g_label ? 1.0 : 0.0 );
		}
	...
	else if (g_state == UPDATE_INPUT)
	{
		for (yi = 0; yi < hi; yi++)
			for (xi = 0; xi < wi; xi++)
				nl_input->neuron_vector[yi * wi + xi].output.ival = (int) (network->get_visible_neuron_value(yi * wi + xi) * 255.0);

		if (get_input_by_neural_layer(nl_input))
			update_input_image(get_input_by_neural_layer(nl_input));
	}
	else if (g_state == UNLOAD_LEARNING)
	{
		network->unload(nl_output->name);
	}
	else if (g_state == RELOAD_LEARNING)
	{
		network->reload(nl_output->name);
	}
	else if (g_state == BACK_PROP_INIT)
	{
		if (g_DBN == NULL)
		{
			unsigned int layers_dimentions[5] = {
					(unsigned int)character.neuron_layer->dimentions.x * character.neuron_layer->dimentions.y, // input
					(unsigned int)character_rbm1.dimentions.x * character_rbm1.dimentions.y,
					(unsigned int)character_rbm2.dimentions.x * character_rbm2.dimentions.y,
					(unsigned int)character_rbm3.dimentions.x * character_rbm3.dimentions.y,
					(unsigned int)character_rbm4.dimentions.x * character_rbm4.dimentions.y // output
			};
			g_DBN = fann_create_standard_array(5, layers_dimentions);
			fann_set_activation_function_hidden(g_DBN, FANN_SIGMOID);
			fann_set_activation_function_output(g_DBN, FANN_SIGMOID);
			fann_set_training_algorithm(g_DBN, FANN_TRAIN_RPROP);
			fann_set_learning_rate(g_DBN, 0.1);
			fann_set_learning_momentum(g_DBN, 0.6);
		}
		//#define DEBUG 1
#ifdef DEBUG
		struct fann_connection *connections;
		int total_connections = fann_get_total_connections(g_DBN);
		connections = (struct fann_connection*) malloc(sizeof(struct fann_connection) * total_connections);

		//fann_print_parameters(g_DBN);
		//fann_print_connections(g_DBN);
		printf("total connections: %i\n", total_connections);

		fann_get_connection_array(g_DBN, connections);
#endif

		int cc = g_previous_connections;
		for (int j = 0; j < wo * ho; j++)
		{
//			uint jj = g_previous_neurons + j + (wi * hi + 1);
			for (int i = 0; i < wi * hi; i++)
			{
//				uint ii = g_previous_neurons + i;
#ifdef DEBUG
				if (connections[cc].from_neuron == ii && connections[cc].to_neuron == jj)
					connections[cc].weight = network->get_synapse_weight_value(i, j);
				else
					printf("Matching error: i = %d, j = %d\n", i, j);
#endif
				//FIXME: muito lenta a chamada fann_set_weight(g_DBN, ii, jj, network->get_synapse_weight_value(i, j));
				g_DBN->weights[cc] = network->get_synapse_weight_value(i, j);
				cc++;
			}
			cc++; //bias neuron
		}
		g_previous_neurons += (wi * hi + 1);
		g_previous_connections += (wi * hi + 1) * (wo * ho);
	...
	else if (g_state == BACK_PROP_TRAIN)
	{
		struct 	fann_train_data *train_data;

		train_data = fann_read_train_from_array((fann_type*)g_input_data, (fann_type*)g_output_data, 60000, 28*28, 10);

		fann_train_on_data(g_DBN, train_data, 200, 10, 1.5);
	...
	else if (g_state == SET_AND_TEST)
	{
		...
		for (yi = 0; yi < character.neuron_layer->dimentions.y; yi++)
			for (xi = 0; xi < character.neuron_layer->dimentions.x; xi++)
				input[yi * character.neuron_layer->dimentions.x + xi] =
						((fann_type)character.neuron_layer->neuron_vector[yi * character.neuron_layer->dimentions.x + xi].output.ival) / 255.0;

		output = fann_run(g_DBN, input);

		for (yo = 0; yo < character_rbm4.dimentions.y; yo++)
			for (xo = 0; xo < character_rbm4.dimentions.x; xo++)
				character_rbm4.neuron_vector[yo * character_rbm4.dimentions.x + xo].output.fval =
						(float) (output[yo * character_rbm4.dimentions.x + xo]);

		update_output_image(get_output_by_neural_layer(&character_rbm4));
	}
}
