...
int init_user_functions (void)
{
	...
	initialize_core_server ("WNN_COR");
...
void draw_output (char *strOutputName, char *strInputName)
{
  	...
  	if ((pFile = fopen (strFileName, "r")) != NULL)
    	{  
      		read_cnae_classes(pFile,&entry);
      		entryId = search_neurons_output_table (&entry);

      		// If an identical entry was not found, insert the new entry in the last position of the neuron's output table
      		if (entryId<0)
		{
	  		entryId = insert_neurons_output_table (&entry);
	  		printf("entry= %i\n", entryId);
		}
  	...
  	set_neurons (output->neuron_layer->neuron_vector, 0, output->wh * output->ww, entryId ); 
...
EvaluateOutput (OUTPUT_DESC *output)
{
	...
	if(classify_flag)
	{
  	...
  	for (i = 0; i < (output->wh * output->ww); i++)
    	{
       		entryId = neuron_vector[i].output.ival;
		nMaxI = 0;

       		for (j = 0; j < g_neuronsOutputTable.entry[entryId].nClasses; j++)
	 	{
			cnae = get_cnae_by_cnae_table_order (g_neuronsOutputTable.entry[entryId].classes[j]);
			...
			for (k = i + 1; k < (output->wh * output->ww); k++)
			{
				...
				for (l = 0; l < g_neuronsOutputTable.entry[entryId].nClasses; l++)
				{
					cnae = get_cnae_by_cnae_table_order (g_neuronsOutputTable.entry[entryId].classes[l]);
			...
			if(classify_flag)
			{
				if(nAux > nMaxI)
				{
					...
					(seq_code_vec_aux)[i] = g_neuronsOutputTable.entry[entryId].classes[j];
					nMaxI = nAux;
				}
			}
		}
