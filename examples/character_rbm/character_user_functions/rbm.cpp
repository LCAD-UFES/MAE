...
BoltzmannMachine::BoltzmannMachine(int _visible_neurons, int _hidden_neurons)
: visible_neurons(_visible_neurons), hidden_neurons(_hidden_neurons), type(SIGMOID)
{
...
double
BoltzmannMachine::get_synapse_weight_value(uint i, uint j) const
{
	if ( (i < visible_neurons && i > 0) && (j < hidden_neurons && j > 0) )
	{
		return synapse_weights[i * hidden_neurons + j];
	}
	return 0.0;
}
...
void
multiply_A_by_B(double *C, double *A, double *B, uint a_rows, uint a_cols_and_b_rows, uint b_cols)
{
	uint i, j, k;
	double sum;
	...
	{
		#ifdef _OPENMP
		#pragma omp parallel for default(none) private(j,k,sum) shared(C,A,B,i,b_cols,a_cols_and_b_rows)
		#endif
		for (j = 0; j < b_cols; j++)
		{
			sum = 0.0;
			for (k = 0; k < a_cols_and_b_rows; k++)
				sum += A[i * a_cols_and_b_rows + k] * B[k * b_cols + j];
...
void
multiply_A_by_B_transpose(double *C, double *A, double *B, uint a_rows, uint a_cols_and_b_rows, uint b_cols)
{
	uint i, j, k;
	double sum;
	...
	for (i = 0; i < a_rows; i++)
	{
		...
		for (j = 0; j < b_cols; j++)
		{
			sum = 0.0;
			for (k = 0; k < a_cols_and_b_rows; k++)
				sum += A[i * a_cols_and_b_rows + k] * B[j * a_cols_and_b_rows + k];
...
uint
BoltzmannMachine::num_hidden() const
{
...
uint
BoltzmannMachine::num_visible() const
{
...
uint
BoltzmannMachine::get_type() const
{
	return type;
}
