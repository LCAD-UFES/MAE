...
LoadDataToInput(INPUT_DESC *input)
{
	...
	for ( y = 0; y < y_dimension; y++ )
	{
		...
		{
			input->neuron_layer->neuron_vector[y * x_dimension + x].output.ival = 0;
		}
	}
	...
	for ( x = 0; x < x_dimension; x++ )
	{
		...
		{
            int k_ask = (g_book_data[sample].ask[y].prc - prc_ask_zero) / STEP_PRC_MIN;
            int k_bid = (prc_ask_zero - g_book_data[sample].bid[y].prc) / STEP_PRC_MIN;
...
compute_capital_evolution(OUTPUT_DESC *neural_prediction, OUTPUT_DESC *actual_result)
{
	...
	else
	{
		if ( ( (s1 == 0) && (expected_result_sell_buy > 0.0) && (g_confidence_up > CERTAINTY) && (g_use_confiance == 1) ) ||
				( g_runing_sum_size > 0 &&  (s1 == 0) && (expected_result_sell_buy > 0.0) && (g_mean_correct_negative_pred > CERTAINTY) && (g_use_confiance == 0) ) ||
