...
WriteTestResultToCSV (PARAM_LIST *pParamList)
{
	...
	hit_percentage = (float) correct / (float) (correct + incorrect);
	result_file = fopen("character_result.csv","a");
	
	fprintf(result_file,"%d,%d,%f,%f\n",NL_WIDTH,SYNAPSES,GAUSS_RAD,hit_percentage);	
...
PrintStatistics(PARAM_LIST *pParamList)
{
	...
	hit_percent = (float) correct / (float) (correct + incorrect);
	printf("%4d %4d %3.2f\n", (correct + incorrect), correct, hit_percent * 100.0);
...
ComputeAverageTimeForNeuronsFindingNearestPattern (PARAM_LIST *pParamList)
{
	...
	for (i = 0; i < char_discr.num_neurons; i++)
		output.fval += char_discr.neuron_vector[i].time_metrics;

	output.fval /= (float)char_discr.num_neurons;
...
ComputeAverageNeuronLayerHashCollisionsInTrain (PARAM_LIST *pParamList)
{
	...
	output.fval = (float) count_neuron_layer_average_hash_collisions (char_discr.name);
...
ComputeAverageNeuronLayerHashCollisionsInTest (PARAM_LIST *pParamList)
{
	...
	for (i = 0; i < char_discr.num_neurons; i++)
		output.fval += char_discr.neuron_vector[i].hash_collisions;
