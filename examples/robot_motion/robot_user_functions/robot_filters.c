...
void new_map_v1_to_image (int *xi, int *yi, int wi, int hi, int u, int v, int w, int h, int x_center, int y_center, double log_factor)
{
	...
	*xi = (int) (x_center + (d * cos(theta)) + 0.5);
	*yi = (int) (y_center + (d * sin(theta)) + 0.5);
...
void new_map_image_to_v1 (int xi, int yi, int wi, int hi, int *u, int *v, int w, int h, int x_center, int y_center, double log_factor)
{
	...
	*u = (int) ( (double) (w/2) *  ( 1.0 + log(((d / LOG_POLAR_SCALE_FACTOR) / (double) (wi/2) * (log_factor - 1.0) / log_factor ) +
							 (1.0 / log_factor) ) / log(log_factor) ) + 0.5);
...
#define PLOT_DX_DY(OPX1, OPY1, OPX2, OPY2, DX, DY)									\
	if ((((xi OPX1 x_motion) && (yi OPY1 y_motion)) || ((xi OPX2 x_motion) && (yi OPY2 y_motion))) &&		\
	    ((x_motion + DX) >= 0) && ((y_motion + DY) >= 0) && ((x_motion + DX) < wi) && ((y_motion + DY) < hi))	\
		nl_out->neuron_vector[((y_motion + DY) * wi) + (x_motion + DX)].output.ival = PIXEL(255, 255, 0)
...
void draw_arrow (NEURON_LAYER *nl_out, int xi, int yi, int wi, int hi, int x_motion, int y_motion)
{
	...
	if (nPixels)
	{
		...
		for (i = 0, flt_x = (float) xi, flt_y = (float) yi; i < nPixels; i++)
		{
			flt_x += delta_x;
			flt_y += delta_y;
			x = (delta_x >= 0.0 ? (int) (flt_x + 0.5) : (int) (flt_x - 0.5));
			y = (delta_y >= 0.0 ? (int) (flt_y + 0.5) : (int) (flt_y - 0.5));
...
void robot_motion_area_filter (FILTER_DESC *filter_desc)
{
	...
	if (g_Arrows == 2)
	{
		for (x_med = g_Arrow_Grid/2; x_med < wi; x_med += g_Arrow_Grid)
		{
			for (y_med = ((y_med % g_Arrow_Grid) ? 0 : g_Arrow_Grid/2); y_med < hi; y_med += g_Arrow_Grid)
			{
				...
				count = 0;
				out_min = FLT_MAX;
				for (xi = (x_med - g_Arrow_Grid/2); xi < (x_med + g_Arrow_Grid/2) && xi < wi; xi++)
				{
					for (yi = (y_med - g_Arrow_Grid/2); yi < (y_med + g_Arrow_Grid/2) && yi < hi; yi++)
					{
						if (yi < 0)
							continue;
						new_map_image_to_v1 (xi, yi, wi, hi, &u, &v, w, h, x_center, y_center, fltLogFactor);
						...
						if (nl_mtmap->neuron_vector[(v * w) + u].output.fval < out_min)
						{
							out_min = nl_mtmap->neuron_vector[(v * w) + u].output.fval;
							x_motion = (int) nl_disp->neuron_vector[(v * w) + u].output.fval;
							y_motion = (int) nl_vdisp->neuron_vector[(v * w) + u].output.fval;
							count = 1;
						}
					}
				}
				if (count)
				{
					draw_arrow (nl_out, x_med, y_med, wi, hi, x_med + x_motion, y_med + y_motion);
				}
			}
		}
	}
