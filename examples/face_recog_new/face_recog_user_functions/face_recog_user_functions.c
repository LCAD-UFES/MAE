...
int GetGroundTruths (void)
{
	...
	/*LOG_POLAR_SCALE_FACTOR = sqrtf ((float) ((g_nLeftEyeX - g_nRightEyeX) * (g_nLeftEyeX - g_nRightEyeX) + 
						 ...
						 (EYE_BASELINE / FOV_HORIZONTAL * face_recog.neuron_layer->dimentions.x);*/
	
	/*if ((g_nRightEyeX - g_nLeftEyeX) != 0)
		LOG_POLAR_THETA_CORRECTION = atanf ((float) (g_nLeftEyeY - g_nRightEyeY) / (float) (g_nLeftEyeX - g_nRightEyeX));
	else
		LOG_POLAR_THETA_CORRECTION = 0.0;
	
	g_nNoseX = (g_nLeftEyeX + g_nRightEyeX) / 2;
	g_nNoseY = (g_nLeftEyeY + g_nRightEyeY) / 2;*/
	
	/*if (((g_nLeftEyeX + g_nRightEyeX) / 2 - g_nNoseX) != 0)
		LOG_POLAR_THETA_CORRECTION = -atanf ((float) ((g_nLeftEyeX + g_nRightEyeX) / 2 - g_nNoseX) / 
						     (float) ((g_nLeftEyeY + g_nRightEyeY) / 2 - g_nNoseY));
	else
		LOG_POLAR_THETA_CORRECTION = 0.0;*/
