...
int
LoadImageToInput(INPUT_DESC *input, char *image)
{
	int x, y;
	...
	for (y = 0; y < input->neuron_layer->dimentions.y; y++)
	{
		...
		{
			input->neuron_layer->neuron_vector[y * input->neuron_layer->dimentions.x + x].output.fval = 
				PIXEL(image[3 * (y * input->neuron_layer->dimentions.x + x) + 2], 
				      image[3 * (y * input->neuron_layer->dimentions.x + x) + 1],
				      image[3 * (y * input->neuron_layer->dimentions.x + x) + 0]);
		}
	}
...
int
LoadRewardToOutput(OUTPUT_DESC *output, float reward)
{
	int x, y;
	...
	for (y = 0; y < output->neuron_layer->dimentions.y; y++)
	{
		...
		{
			output->neuron_layer->neuron_vector[y * output->neuron_layer->dimentions.x + x].output.fval = reward;
		}
	}
...
float
GetRewardOutput(OUTPUT_DESC *output)
{
	int x, y;
...
init_ita(INPUT_DESC *input)
{
	...
	char *image = malloc(160*160*3);
	int m;
	for (m = 0; m < 160*160*3; m++)
		image[m] = rand();
	LoadImageToInput(&ita, image);
